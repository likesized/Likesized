...
export type QuickViewDetail = { label:string; value:string|number|null|undefined };
type Props={kind:"person"|"garment"|"outfit";title:string;subtitle?:string|null;imageUrl?:string|null;description?:string|null;details?:QuickViewDetail[];href:string;fullLabel:string;children:ReactNode;inline?:boolean;ariaLabel?:string};

export function EntityQuickView({kind,title,subtitle,imageUrl,description,details=[],href,fullLabel,children,inline=false,ariaLabel}:Props){
  ...
  return <>
    <button type="button" className={`${styles.trigger}${inline?` ${styles.triggerInline}`:""}`} aria-label={ariaLabel??`Quick view ${title}`} onClick={()=>setOpen(true)}>{children}</button>
    {open?<div className={styles.overlay} role="dialog" aria-modal="true" aria-label={`${title} quick view`} onClick={()=>setOpen(false)}><section className={styles.card} onClick={(event)=>event.stopPropagation()}>
