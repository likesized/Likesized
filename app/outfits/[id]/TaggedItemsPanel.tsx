...
export default function TaggedItemsPanel({items,postId,signedIn}:{items:TaggedItem[];postId:string;signedIn:boolean}){
  ...
  const bestRelevantReport=meta?.relevantReports.length?[...meta.relevantReports].sort((a,b)=>(b.bodyMatch??-1)-(a.bodyMatch??-1)||Number(b.isOwn)-Number(a.isOwn))[0]:null;
  ...
  return <div className={styles.taggedPanel}>
    ...
    {selected?<div className={styles.itemPreviewOverlay} role="dialog" aria-modal="true" aria-label={`${selected.label} quick view`} onClick={()=>setSelectedId(null)}><div className={styles.itemPreviewCard} onClick={(event)=>event.stopPropagation()}>
      ...
      {evidenceOpen&&meta?<>
        ...
        <div className={quickStyles.detailHeader}><span className={styles.fituitionKicker}>FITuition DETAILS</span><strong>{selected.label}</strong><span>{meta.variationDetail||meta.category}</span></div>
        {showRecommendation&&meta.recommendation?<>
          <section className={quickStyles.detailSummary} aria-label="FITuition recommendation summary">
            <span>Recommended size</span>
            <div className={quickStyles.detailRecommendation}><strong>{meta.recommendation.sizeLabel}</strong><span>{meta.recommendation.confidenceLabel}</span></div>
            <small>Based on {meta.matchingFitReports} Relevant Fit Report{meta.matchingFitReports===1?"":"s"} plus the supporting evidence FITuition can use for you.</small>
          </section>
          {meta.strongFitReports.length?<section className={quickStyles.evidenceSection}><div className={quickStyles.sectionHeading}><strong>Strong Fit Report summary</strong><span>Exact tracked variation</span></div><div className={styles.relevantReportList}>{meta.strongFitReports.map((group)=><div className={styles.relevantReport} key={group.sizeLabel}><div><strong>Size {group.sizeLabel}</strong><span>{group.count} strong report{group.count===1?"":"s"}</span></div>{group.fitBreakdown.map((fit)=><div key={fit.fit}><span>{fit.fitLabel}</span><strong>{fit.percent}%</strong></div>)}</div>)}</div></section>:bestRelevantReport?<section className={quickStyles.evidenceSection}><div className={quickStyles.sectionHeading}><strong>Strongest current evidence</strong><span>Exact tracked variation</span></div><div className={styles.relevantReportList}><div className={styles.relevantReport}><div>{bestRelevantReport.bodyMatch===null?<><strong>Your Fit Report</strong><span>Your own exact report</span></>:<><strong>{bestRelevantReport.bodyMatch}%</strong><span>Body Match{bestRelevantReport.isOwn?" · You":""}</span></>}</div><div><span>Size</span><strong>{bestRelevantReport.sizeLabel}</strong></div><div><span>Fit Result</span><strong>{bestRelevantReport.fitLabel}</strong></div></div></div></section>:null}
        </>:meta.relevantReports.length?<>
          <section className={quickStyles.detailSummary} aria-label="FITuition evidence summary">
            <strong>FITuition still can’t recommend a size yet.</strong>
            <span>Relevant Fit Reports: {meta.matchingFitReports}</span>
            <small>The available exact-variation evidence does not point clearly enough to one size yet.</small>
          </section>
          {bestRelevantReport?<section className={quickStyles.evidenceSection}><div className={quickStyles.sectionHeading}><strong>{meta.relevantReports.length>1?"Best current match":"Available Fit Report"}</strong><span>Exact tracked variation</span></div><div className={styles.relevantReportList}><div className={styles.relevantReport}><div>{bestRelevantReport.bodyMatch===null?<><strong>Your Fit Report</strong><span>Your own exact report</span></>:<><strong>{bestRelevantReport.bodyMatch}%</strong><span>Body Match{bestRelevantReport.isOwn?" · You":""}</span></>}</div><div><span>Size</span><strong>{bestRelevantReport.sizeLabel}</strong></div><div><span>Fit Result</span><strong>{bestRelevantReport.fitLabel}</strong></div></div></div></section>:null}
        </>:<section className={quickStyles.detailSummary}><strong>No Relevant Fit Reports yet.</strong><span>Return to FITuition to turn on notifications.</span></section>}
        <Link prefetch={false} className={quickStyles.fullEvidenceLink} href={selected.href} data-full-navigation="true">{!showRecommendation&&meta.relevantReports.length>1?`View all ${meta.relevantReports.length} Relevant Fit Reports →`:"View Garment Details →"}</Link>
      </>:<>
