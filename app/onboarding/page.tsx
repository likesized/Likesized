...
import { FitProfileForm, type BodyMeasurement, type GarmentFitPreference, type GarmentTypePreferenceOption, type MeasurementType } from "@/app/onboarding/FitProfileForm";
...
export default async function OnboardingPage({searchParams}:{searchParams:SearchParams}){
  ...
  const [{data:profile},{data:fitProfile},{data:types},{data:measurements},{data:garmentTypes},{data:fitPreferences}]=await Promise.all([
  ...
  return <main className="onboardingShell">
    ...
    <FitProfileForm username={profile?.username??""} isInitialSetup={isInitialSetup} unitSystem={unitSystem} types={(types??[]) as MeasurementType[]} measurements={(measurements??[]) as BodyMeasurement[]} garmentTypes={(garmentTypes??[]) as GarmentTypePreferenceOption[]} fitPreferences={(fitPreferences??[]) as GarmentFitPreference[]} errorMessage={errorMessage}/>
  </main>;
}
