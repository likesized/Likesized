...
test("Outfit occasion and style chips navigate straight to filtered Explore with exact return context",()=>{
 assert.match(detail,/\/explore\?view=outfits&scope=all&occasion=/);assert.match(detail,/\/explore\?view=outfits&scope=all&style=/);assert.match(detail,/return_to=\$\{exploreReturn\}/);assert.match(detail,/prefetch=\{false\}/);assert.match(explore,/← Back to Outfit/);assert.match(explore,/Showing Outfits tagged/);assert.match(explore,/outfit_occasions/);assert.match(explore,/outfit_style_tags/);
});
...
test("Strong exact Fit Reports and Closet history both feed the one tagged FITuition engine",()=>{
 assert.match(tagged,/Strong exact Fit Reports:/);assert.match(tagged,/meta\?\.category/);assert.match(tagged,/item\.imageUrl/);assert.doesNotMatch(taggedFit,/get_product_fit_summary/);assert.match(taggedFit,/relevantExact/);assert.match(taggedFit,/historical_match_score>=STRONG_FIT_REPORT_MATCH_THRESHOLD/);assert.match(taggedFit,/get_fit_report_snapshot_matches/);assert.match(taggedFit,/closetEvidenceRelevance/);assert.match(taggedFit,/source:"community"/);assert.match(taggedFit,/source:"closet"/);assert.match(taggedFit,/recommendSize\(\[\.\.\.otherEvidence,\.\.\.ownHistory\]\)/);
});

test("Tagged quick view resolves loading, exposes retry, hybrid evidence, and the account gate",()=>{
 assert.match(tagged,/new AbortController\(\)/);assert.match(tagged,/fitErrors/);assert.match(tagged,/FITuition couldn’t load this evidence/);assert.match(tagged,/>Try again<\/button>/);assert.doesNotMatch(tagged,/\[selected\?\.closetItemId,signedIn,postId,fitMeta,fitLoading\]/);assert.match(tagged,/Our FITuition suggests/);assert.match(tagged,/Mixed evidence/);assert.match(tagged,/See Relevant Fit Reports →/);assert.match(tagged,/View Detailed Garment Report →/);assert.match(tagged,/UniversalActionButton action="likeLocker"/);assert.match(tagged,/UniversalActionButton action="wishLocker"/);assert.match(tagged,/UniversalActionLink action="shop"/);assert.match(tagged,/UniversalActionButton action="share"/);assert.match(tagged,/UniversalActionSummary action="report"/);assert.match(universalActions,/wishLocker:\s*\{[^}]*label:\s*"Wishlist"/);assert.match(tagged,/Create account/);assert.match(tagged,/sign in to see Matching Fit Reports, Body Match, and FITuition/);assert.match(lockerActions,/stayOpen/);assert.match(lockerActions,/if \(stayOpen\) return/);assert.match(taggedFit,/recommendation&&recommendation\.confidence>=45/);assert.doesNotMatch(taggedFit,/recommendation\.confidence>=45&&relevantExact\.length>0/);
});
