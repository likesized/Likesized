...
export default async function ExplorePage({searchParams}:{searchParams:SearchParams}){
  ...
  let outfits:Outfit[]=[];
  if(view==="outfits"){
    let query=supabase.from("outfit_posts").select("id,user_id,headline,photo_url,created_at,profile:profiles(username,display_name,avatar_url)").eq("status","published").order("created_at",{ascending:false}).limit(resultLimit);
