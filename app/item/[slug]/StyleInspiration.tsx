...
export default async function StyleInspiration({productId,variationKey}:{productId:string;variationKey:string|null}){
  ...
  let reportsQuery=supabase.from("fit_reports").select("closet_item_id").eq("product_id",productId).not("closet_item_id","is",null).order("created_at",{ascending:false}).limit(MAX_REPORTS);
  if(variationKey)reportsQuery=reportsQuery.eq("tracked_variation_key",variationKey);
  const reportResult=await reportsQuery;
  if(reportResult.error)return null;
  const closetIds=[...new Set(((reportResult.data??[]) as ReportRow[]).map((row)=>row.closet_item_id).filter(Boolean))];
  if(!closetIds.length)return null;

  const linkResult=await supabase.from("outfit_post_items").select("post_id").in("closet_item_id",closetIds).limit(MAX_POSTS*4);
  if(linkResult.error)return null;
  const postIds=[...new Set(((linkResult.data??[]) as LinkRow[]).map((row)=>row.post_id))].slice(0,MAX_POSTS*2);
  if(!postIds.length)return null;

  const cutoff=new Date(Date.now()-LOOKBACK_DAYS*24*60*60*1000).toISOString();
  const postResult=await supabase.from("outfit_posts").select("id,photo_url,like_count,comment_count,share_count,published_at,created_at").in("id",postIds).eq("status","published").gte("published_at",cutoff).limit(MAX_POSTS);
  ...
  return <section className={styles.inspirationSection}>
    ...
    <div className={styles.inspirationGrid}>{displayed.map((post)=>{
      ...
      return <Link prefetch={false} className={styles.inspirationCard} href={`/outfits/${post.id}`} key={post.id}>{url?<img src={url} alt="Tagged Outfit inspiration" loading="lazy"/>:<span>OUTFIT</span>}</Link>;
    })}</div>
  </section>;
}
