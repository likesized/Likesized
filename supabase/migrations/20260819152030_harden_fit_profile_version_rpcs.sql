-- LikeSized canonical migration: 20260819152030_harden_fit_profile_version_rpcs
-- Exact SQL applied to the connected Supabase project.

-- Keep public Data API RPCs SECURITY INVOKER. A narrow private helper owns immutable-version creation and always derives the user from auth.uid().

create or replace function private.ensure_current_fit_profile_version(
...
begin
  ...
  return private.ensure_fit_profile_version_for_user(v_user_id,p_reason);
end;
$$;
revoke all on function private.ensure_current_fit_profile_version(text) from public,anon;
grant execute on function private.ensure_current_fit_profile_version(text) to authenticated;
...
set search_path=''
as $$
  select private.ensure_current_fit_profile_version('fit_report_lock');
$$;
...
comment on function public.commit_fit_profile_version() is
  'SECURITY INVOKER public wrapper. Locks/reuses the signed-in user current immutable body-state version through a private auth.uid()-bound helper.';
comment on function public.save_fit_profile(text,public.unit_system,jsonb) is
  'SECURITY INVOKER atomic canonical Fit Profile save under RLS, followed by private auth.uid()-bound immutable version creation.';
