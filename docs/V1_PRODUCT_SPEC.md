...
- With **one or more Relevant Fit Reports but insufficient recommendation confidence**, the quick view uses the owner-approved wording **“Not enough fit data to confidently recommend a size.”**, shows **Relevant Fit Reports: X**, and offers one **FITuition Details** action. With enough strong evidence to recommend, the quick view uses **“Our FITuition suggests: [SIZE]”**, shows **“Confidence: [label]”**, and offers that same FITuition Details action.
- The first clicked tagged-garment quick view always keeps a direct **View Garment Detail →** link at the bottom, regardless of FITuition state, including the zero-report Notify state. **FITuition Details remains the intermediate in-Outfit evidence layer, not immediate full-page navigation.** The FITuition Details click keeps the viewer in the Outfit context and does not repeat the quick-view recommendation/count summary. In a low-confidence state it shows the single best/closest exact report first and a **View more Relevant Fit Reports →** control when additional exact reports exist. In a recommendation state it shows compact strong exact-report aggregate rows as count + Size + Fit Result. The intermediate layer also contains explicit **View Garment Detail →** navigation. The full Garment Detail page is where the broader evidence set, including related/similar tracked variations, belongs.
