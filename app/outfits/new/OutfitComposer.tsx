...
export default function OutfitComposer({ closet, initial, styleSuggestions = [] }: { closet: ClosetOption[]; initial?: InitialOutfit | null; styleSuggestions?: string[] }) {
  ...
  const [photoSuccess, setPhotoSuccess] = useState<"cover"|"additional"|null>(null);
  ...
  function continueAfterPhotoUpload(){setPhotoStatus("");setPhotoSuccess(null);postSectionRef.current?.scrollIntoView({behavior:"smooth",block:"start"});}
  ...
  async function chooseCover(file?: File) {
    if (!file||mainPhoto) return; setBusyPhotos(true); setPhotoSuccess(null); setPhotoStatus("Preparing cover photo…");
    try { const next = await makePhoto(file, true); setPhotos((current) => [next, ...current.map((photo) => ({ ...photo, isMain: false }))].slice(0, 6)); setPhotoStatus("Cover photo uploaded successfully."); setPhotoSuccess("cover"); markPhotos(); }
    catch (error) { setPhotoSuccess(null); setPhotoStatus(error instanceof Error ? error.message : "Photo preparation failed."); }
    finally { setBusyPhotos(false); }
  }
  async function chooseAdditional(files: FileList | null) {
    if (!files?.length) return; setBusyPhotos(true); setPhotoSuccess(null); setPhotoStatus("Preparing additional photos…");
    try { const available = Math.max(0, 6 - photos.length); const chosen = [...files].slice(0, available); const settled = await Promise.allSettled(chosen.map((file, index) => makePhoto(file, photos.length === 0 && index === 0))); const next = settled.flatMap((result) => result.status === "fulfilled" ? [result.value] : []); const failed = settled.length - next.length; if (next.length) { setPhotos((current) => [...current, ...next].slice(0, 6)); markPhotos(); setPhotoSuccess("additional"); setPhotoStatus(failed ? `${next.length} ${next.length === 1 ? "photo" : "photos"} uploaded successfully. ${failed} could not be prepared.` : next.length === 1 ? "Photo uploaded successfully." : `${next.length} photos uploaded successfully.`); } else { setPhotoSuccess(null); setPhotoStatus(failed ? `${failed} ${failed === 1 ? "photo" : "photos"} could not be prepared.` : "No photos were added."); } }
    catch (error) { setPhotoSuccess(null); setPhotoStatus(error instanceof Error ? error.message : "Photo preparation failed."); }
    finally { setBusyPhotos(false); }
  }
  ...
  function removePhoto(key: string) { setPhotoStatus(""); setPhotoSuccess(null); setPhotos((current) => { const removed = current.find((photo) => photo.key === key); if (removed?.previewUrl.startsWith("blob:")) URL.revokeObjectURL(removed.previewUrl); const next = current.filter((photo) => photo.key !== key); if (removed?.isMain && next.length) next[0] = { ...next[0], isMain: true }; return next; }); markPhotos(); }
  function setCover(key: string) { setPhotoStatus(""); setPhotoSuccess(null); setPhotos((current) => { const chosen = current.find((photo) => photo.key === key); if (!chosen) return current; return [{ ...chosen, isMain: true }, ...current.filter((photo) => photo.key !== key).map((photo) => ({ ...photo, isMain: false }))]; }); markPhotos(); }
  ...
  return <>
    <section className={styles.composer}>
      <div className={styles.sectionCard}>
        ...
        <div className={styles.uploadGrid}>{mainPhoto?<div className={styles.uploadComplete}><strong>Cover photo</strong><span>Cover photo is set.</span></div>:<label className={styles.uploadBox}><strong>Cover photo (required)</strong><input type="file" accept="image/*" disabled={busyPhotos} onChange={(event) => { void chooseCover(event.currentTarget.files?.[0]); event.currentTarget.value = ""; }} /></label>}<label className={styles.uploadBox}><strong>Additional photos (optional)</strong><span>Upload up to 5 additional photos.</span><input ref={additionalInputRef} type="file" multiple accept="image/*" disabled={busyPhotos || photos.length >= 6} onChange={(event) => { void chooseAdditional(event.currentTarget.files); event.currentTarget.value = ""; }} /></label></div>
        {photoStatus ? <div className={photoSuccess?styles.uploadFeedback:"fieldHelp"} aria-live="polite"><strong>{photoStatus}</strong>{photoSuccess==="additional"?<div className={styles.uploadSuccessActions}>{photos.length<6?<button type="button" className={styles.compactSecondary} disabled={busyPhotos} onClick={()=>additionalInputRef.current?.click()}>Add more</button>:null}<button type="button" className={styles.quietButton} onClick={continueAfterPhotoUpload}>Continue editing the Outfit</button></div>:null}</div> : null}
      ...
      <div className={styles.sectionCard} ref={postSectionRef}><div className={styles.sectionHeading}><div><span className="eyebrow">2 · THE POST</span><h2>Tell people about the look.</h2></div></div><label>Headline <span className={styles.counter}>{headline.length}/100</span><input value={headline} maxLength={100} required placeholder="Give your outfit a title" onChange={(event) => { setHeadline(event.target.value); mark(); }} /></label><label>Outfit Story <span className="muted inlineMuted">optional</span> <span className={styles.counter}>{story.length}/5,000</span><textarea value={story} rows={7} maxLength={5000} placeholder="Share the details, inspiration, styling choices, or anything else you want people to know about the look." onChange={(event) => { setStory(event.target.value); mark(); }} /></label></div>
