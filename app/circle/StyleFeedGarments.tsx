...
export default function StyleFeedGarments({postId}:{postId:string}){
  ...
  return <div ref={hostRef} onPointerEnter={warm} onPointerDown={warm} onFocusCapture={warm}>{items?.length?<StyleFeedGarmentsButton items={items} postId={postId}/>:failed?<button type="button" onClick={()=>{setFailed(false);void loadItems(postId).then(setItems).catch(()=>setFailed(true));}}>View Garments →</button>:null}</div>;
}
