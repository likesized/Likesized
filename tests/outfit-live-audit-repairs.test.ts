...
test("own exact-item history counts as visible exact-variation Relevant evidence and still feeds recommendation",()=>{
 assert.match(taggedFit,/targetReport\.user_id===viewerId/);assert.match(taggedFit,/const ownHistory:RecommendationEvidence\[\]/);assert.match(taggedFit,/source:"closet" as const/);assert.match(taggedFit,/recommendSize\(\[\.\.\.otherEvidence,\.\.\.ownHistory\]\)/);assert.match(taggedFit,/const otherRelevantExact=relevantExact\.filter\(\(row\)=>row\.user_id!==viewerId\)/);assert.match(taggedFit,/const ownExactReports:RelevantReport\[\]=ownReports\.filter/);assert.match(taggedFit,/isOwn:true/);assert.match(taggedFit,/const relevantReports=\[\.\.\.ownExactReports,\.\.\.strongOtherReports\]/);assert.match(taggedFit,/matchingFitReports:relevantReports\.length/);assert.match(taggedFit,/strongFitReports:strongAggregate\(otherRelevantExact/);assert.doesNotMatch(taggedFit,/const relevantReports=strongOtherReports/);
});
