...
export default async function EditClosetItemPage({params,searchParams}:{params:Params;searchParams:SearchParams}){
  ...
  const [{data:item,error:itemError},{data:reports,error:reportError},{data:photo,error:photoError}]=await Promise.all([
  ...
  const errorMessage=error==="photo_requires_shared"?"A fit/reference photo is always shared. Remove the fit photo before making this Closet item private.":error==="invalid_settings"?"Check the Closet settings.":error==="invalid_observation"?"Check the fit observation.":error==="confirm_delete"?"Type DELETE exactly to remove this garment and its fit history.":error==="save_failed"?"That change could not be saved.":null;
  ...
  return <main className="pageShell">
    ...
    <section className="section"><div className="sectionHeading"><div><span className="eyebrow">FIT NOW</span><h2>Log a new fit observation</h2><p>This records how the same garment fits your current body. It creates a new immutable Fit Report and leaves every older observation unchanged.</p></div></div>
      <form className="garmentForm" action={logFitObservation}><input type="hidden" name="closet_item_id" value={id}/><div className="fieldPair"><label>Overall fit<select name="fit" defaultValue="" required><option value="" disabled>Select fit</option><option value="too_small">Too small</option><option value="snug">Snug</option><option value="just_right">Just right</option><option value="relaxed">Relaxed</option><option value="too_big">Too big</option></select></label><label>Would you buy it again?<select name="would_buy_again" defaultValue="unsure"><option value="yes">Yes</option><option value="no">No</option><option value="unsure">Not sure</option></select></label></div><label>Fit notes <span className="muted inlineMuted">optional</span><textarea name="fit_notes" maxLength={1000} rows={4} placeholder="How does it fit now?"/></label><button className="primaryButton" type="submit">Save new fit observation</button></form>
    </section>
