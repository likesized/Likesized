...
test("own exact-item history counts as visible exact-variation Relevant evidence and still feeds recommendation",()=>{
 assert.match(taggedFit,/targetReport\.user_id===viewerId/);assert.match(taggedFit,/const ownHistory:RecommendationEvidence\[\]/);assert.match(taggedFit,/source:"closet" as const/);assert.match(taggedFit,/recommendSize\(\[\.\.\.otherEvidence,\.\.\.ownHistory\]\)/);assert.match(taggedFit,/const otherRelevantExact=relevantExact\.filter\(\(row\)=>row\.user_id!==viewerId\)/);assert.match(taggedFit,/const ownRelevantExact=ownReports\.filter/);assert.match(taggedFit,/const ownExactReports:RelevantReport\[\]=ownRelevantExact\.map/);assert.match(taggedFit,/isOwn:true/);assert.match(taggedFit,/const relevantReports=\[\.\.\.ownExactReports,\.\.\.strongOtherReports\]/);assert.match(taggedFit,/matchingFitReports:relevantReports\.length/);assert.match(taggedFit,/const strongExactEvidence:StrongAggregateEvidence\[\]=\[/);assert.match(taggedFit,/\.\.\.ownRelevantExact\.map/);assert.match(taggedFit,/\.\.\.otherRelevantExact\.map/);assert.match(taggedFit,/strongFitReports:strongAggregate\(strongExactEvidence\)/);assert.doesNotMatch(taggedFit,/strongFitReports:strongAggregate\(otherRelevantExact/);assert.doesNotMatch(taggedFit,/const relevantReports=strongOtherReports/);
});

test("Tagged quick view resolves loading, persists notification state, and uses plain Relevant Fit Report copy",()=>{
 assert.match(tagged,/new AbortController\(\)/);assert.match(tagged,/fitErrors/);assert.match(fitCard,/FITuition couldn’t load this evidence/);assert.match(fitCard,/>Try again<\/button>/);assert.match(tagged,/loadedFitMeta=useRef\(new Set<string>\(\)\)/);assert.match(tagged,/loadingFitMeta=useRef\(new Set<string>\(\)\)/);assert.match(tagged,/const loadFitMeta=useCallback/);assert.match(tagged,/loadedFitMeta\.current\.add\(closetItemId\)/);assert.match(tagged,/loadFitSummaryBatch\(postId\)/);assert.match(tagged,/tagged-fit-summary/);assert.match(tagged,/fitSummaryCache/);assert.match(tagged,/void loadFitMeta\(selectedId,controller\.signal\)/);assert.doesNotMatch(tagged,/items\.forEach\(\(item,index\)=>\{void loadFitMeta\(item\.closetItemId,controllers\[index\]\.signal\);\}\)/);assert.match(tagged,/void loadFitMeta\(item\.closetItemId,undefined,true\)/);assert.doesNotMatch(tagged,/if\(!selectedId\|\|!signedIn\|\|loadedFitMeta/);assert.match(fitCard,/Our FITuition suggests/);assert.match(fitCard,/Mixed evidence/);assert.match(tagged,/See Full Details →/);assert.doesNotMatch(tagged,/See FITuition Details →|View more Relevant Fit Reports →|View Garment Detail →|View Garment Details →|View Detailed Garment Report/);assert.match(tagged,/Relevant Fit Reports: Checking…/);assert.match(tagged,/"Notify me"/);assert.match(tagged,/FITuition will notify you when people close to your size post a Fit Report for this item\./);assert.match(tagged,/Notifications on/);assert.match(tagged,/tagged-fit\/watch\?closet_item_id=/);assert.match(taggedWatch,/export async function GET/);assert.match(taggedWatch,/\.eq\("active",true\)/);assert.match(taggedCss,/\.notifyPrompt\{/);assert.match(taggedCss,/\.watchState\{/);assert.doesNotMatch(taggedCss,/\.notifyButton\{/);assert.match(tagged,/UniversalActionButton action="likeLocker"/);assert.match(tagged,/UniversalActionButton action="wishLocker"/);assert.match(tagged,/UniversalActionLink action="shop"/);assert.match(tagged,/UniversalActionButton action="share"/);assert.match(tagged,/UniversalActionSummary action="report"/);assert.match(universalActions,/wishLocker:\s*\{[^}]*label:\s*"Wishlist"/);assert.match(tagged,/Create account/);assert.match(tagged,/sign in to see Matching Fit Reports, Body Match, and FITuition/);assert.match(lockerActions,/stayOpen/);assert.match(lockerActions,/if \(stayOpen\) return/);assert.match(taggedFit,/recommendation&&recommendation\.confidence>=45/);assert.doesNotMatch(taggedFit,/recommendation\.confidence>=45&&relevantExact\.length>0/);
});
