...
export default async function EditClosetItemPage({params,searchParams}:{params:Params;searchParams:SearchParams}){
  ...
  const [{data:item,error:itemError},{data:reports,error:reportError},{data:photoRows,error:photoError},{data:mappings},{data:definitions},{data:responses},{data:unconfirmedData,error:unconfirmedError}]=await Promise.all([
    supabase.from("closet_items").select("id,size_label,wears_count,visibility,product:products(name,slug,garment_type_key,market_segment,brand:brands(name)),submission:garment_submissions(brand_text,model_text,garment_type_key,retailer_url)").eq("id",id).eq("user_id",userId).maybeSingle(),
  ...
  await Promise.all(photos.map(async(photo)=>{const {data}=await supabase.storage.from("fit-reference-photos").createSignedUrl(photo.storage_path,60*30);if(data?.signedUrl)signedPhotos.push({...photo,signedUrl:data.signedUrl});}));
  ...
  return <main className="pageShell">
    ...
    <section className="section"><div className="sectionHeading"><div><span className="eyebrow">FIT NOW</span><h2>Log a new fit observation</h2><p>This records how the same garment physically fits your current body. It creates a new immutable Fit Report and leaves every older observation unchanged.</p></div></div>
      <form className="garmentForm" action={logFitObservation}><input type="hidden" name="closet_item_id" value={id}/>
        ...
        <FitDimensionFields garmentType={effectiveType} dimensions={dimensions} responses={responses??[]}/><label>Fit notes <span className="muted inlineMuted">optional</span><textarea name="fit_notes" maxLength={2000} rows={4} placeholder="How does it fit now?"/></label><button className="primaryButton" type="submit">Save new fit observation</button></form>
    </section>
