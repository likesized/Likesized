...
export default async function ExplorePage({searchParams}:{searchParams:SearchParams}){
  ...
  let outfits:Outfit[]=[];
  if(view==="outfits"){
    const result=await supabase.from("outfit_posts").select("id,user_id,caption,photo_url,created_at,profile:profiles(username,display_name)").order("created_at",{ascending:false}).limit(resultLimit);
