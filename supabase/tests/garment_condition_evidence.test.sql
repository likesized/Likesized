...
select ok(
  position('partition by s.user_id, s.evidence_product_id, coalesce(s.objective_variant_key' in lower(pg_get_functiondef('public.get_product_evidence_candidates(uuid,uuid,integer)'::regprocedure))) > 0,
  'recommendation evidence dedupes by wearer + Product + tracked variation rather than wearer alone'
);
