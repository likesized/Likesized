...
import type { StyleFeedGarmentItem } from "./StyleFeedGarmentsButton";
...
export default function StyleFeedBoard({items}:{items:StyleFeedBoardItem[]}){
  const [activeId,setActiveId]=useState<string|null>(null);const [showMatchHelp,setShowMatchHelp]=useState(false);const [galleryGarments,setGalleryGarments]=useState<StyleFeedGarmentItem[]>([]);const active=items.find((item)=>item.id===activeId)??null;
  useEffect(()=>{setShowMatchHelp(false);setGalleryGarments([]);if(!active)return;const previous=document.body.style.overflow;document.body.style.overflow="hidden";const keydown=(event:KeyboardEvent)=>{if(event.key==="Escape")setActiveId(null);};window.addEventListener("keydown",keydown);return()=>{document.body.style.overflow=previous;window.removeEventListener("keydown",keydown);};},[active]);
  return <>
    ...
    {active?<div className={styles.modal} role="dialog" aria-modal="true" aria-label={`${active.headline} Outfit`} onClick={()=>setActiveId(null)}><article className={styles.modalCard} onClick={(event)=>event.stopPropagation()}><button className={styles.close} type="button" aria-label="Close Outfit" onClick={()=>setActiveId(null)}>×</button><div className={styles.mediaPane}>{active.photos.length?<OutfitGallery photos={active.photos} garments={galleryGarments.map((item)=>({id:item.closetItemId,label:item.label,detail:item.detail,href:item.href,imageUrl:item.imageUrl}))}/>:<span>OUTFIT</span>}</div><div className={styles.detailsPane}>
      <PersonQuickView username={active.creator.username} displayName={active.creator.displayName} avatarUrl={active.creator.avatarUrl} inline><span className={styles.creator}>{active.creator.avatarUrl?<img className={styles.avatar} src={active.creator.avatarUrl} alt=""/>:<span className={styles.avatarFallback}>{active.creator.displayName.slice(0,1).toUpperCase()}</span>}<span className={styles.identity}><strong>{active.creator.displayName}</strong><span>@{active.creator.username}</span></span>{active.twinLabel?<span className={styles.badge}>{active.twinLabel}</span>:null}</span></PersonQuickView>
      ...
      <div className={styles.garments}><StyleFeedGarments postId={active.id} onItems={setGalleryGarments}/></div><Link className={styles.fullOutfit} prefetch={false} href={`/outfits/${active.id}`}>View Full Outfit →</Link>
