...
export async function logFitObservation(formData:FormData){
  ...
  const {data:report,error}=await supabase.from("fit_reports").insert({user_id:userId,closet_item_id:id,product_id:item.product_id,variant_id:item.variant_id,size_label:item.size_label,normalized_size_id:item.normalized_size_id,fit_profile_version_id:versionId,fit,garment_condition:garmentCondition,fit_notes:notes,would_buy_again:wouldBuyAgain}).select("id").single();
