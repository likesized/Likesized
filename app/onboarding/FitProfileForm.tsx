...
type Props={username:string;initialCity:string;initialStateRegion:string;isInitialSetup:boolean;unitSystem:UnitSystem;fitCommunity:FitCommunity;types:MeasurementType[];measurements:BodyMeasurement[];errorMessage:string|null};
...
const HIGH_VALUE_ADVANCED_KEYS=new Set(["high_bust","underbust","lower_pants_waist","high_hip","torso_girth","arm_sleeve_length","neck_collar_circumference","thigh_circumference","front_rise","back_rise","foot_length","foot_width"]);
const REGION_ORDER=["upper_body","torso","lower_body","foot"] as const;
const REGION_LABELS:Record<string,string>={upper_body:"Upper body",torso:"Torso",lower_body:"Lower body",foot:"Feet"};
...
function regionGroups(rows:MeasurementType[]){return REGION_ORDER.map((group)=>({group,label:REGION_LABELS[group],rows:rows.filter((row)=>row.measurement_group===group)})).filter((entry)=>entry.rows.length);}

export function FitProfileForm({username,initialCity,initialStateRegion,isInitialSetup,unitSystem:initialSystem,fitCommunity:initialFitCommunity,types,measurements,errorMessage}:Props){
  ...
  const [stateDraft,setStateDraft]=useState(initialStateRegion);
  ...
  useEffect(()=>{if(!reviewing)return;const frame=requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:"auto"}));return()=>cancelAnimationFrame(frame);},[reviewing]);

  const reviewRows=useMemo<ReviewRow[]>(()=>visibleTypes.flatMap((type)=>{
    const raw=values[type.key];const original=byKey.get(type.key);
    if(!raw){if(!isInitialSetup&&original)return[{key:type.key,label:displayLabel(type),value:"Removed",status:null}];return[];}
  ...
  const mostUseful=advanced.filter((row)=>HIGH_VALUE_ADVANCED_KEYS.has(row.key));
  const additional=advanced.filter((row)=>!HIGH_VALUE_ADVANCED_KEYS.has(row.key));
  ...
  const advancedTier=(title:string,copy:string,rows:MeasurementType[])=><section className={helpStyles.advancedTier}><div className={helpStyles.advancedTierHeading}><h3>{title}</h3><p>{copy}</p></div><div className={helpStyles.advancedGroups}>{regionGroups(rows).map((group)=><section className={helpStyles.advancedGroup} key={group.group}><h4>{group.label}</h4><div className={`fieldPair optionalFields ${helpStyles.advancedGrid}`}>{group.rows.map(field)}</div></section>)}</div></section>;
  ...
  return <form className="fitForm" action={saveFitProfile} onSubmit={(event)=>{if(!reviewing){event.preventDefault();(document.activeElement as HTMLElement|null)?.blur();setReviewing(true);}}}>
    ...
    {reviewing?<>
      ...
      {isInitialSetup?<div className="evidence"><div><strong>City / State</strong><span>{cityDraft&&stateDraft?`${cityDraft}, ${stateDraft}`:"Not provided"}</span></div></div>:null}
      ...
      <input type="hidden" name="username" value={isInitialSetup?usernameDraft:""}/><input type="hidden" name="unit_system" value={system}/>{isInitialSetup?<><input type="hidden" name="fit_community" value={communityDraft}/><input type="hidden" name="city" value={cityDraft}/><input type="hidden" name="state_region" value={stateDraft}/></>:null}{visibleTypes.map((type)=><span key={type.key}><input type="hidden" name={`measurement_${type.key}`} value={values[type.key]??""}/>{confirmedUnchanged.has(type.key)?<input type="hidden" name={`confirm_measurement__${type.key}`} value="1"/>:null}</span>)}
      ...
      {isInitialSetup?<><div className="fieldPair"><label>City<input name="city" type="text" value={cityDraft} onChange={(event)=>setCityDraft(event.target.value)} maxLength={80} autoComplete="address-level2" required/></label><label>State<input name="state_region" type="text" value={stateDraft} onChange={(event)=>setStateDraft(event.target.value)} maxLength={80} autoComplete="address-level1" required/></label></div><span className="fieldHelp">City and state stay private and can be changed later in Settings.</span></>:null}
      ...
      <details open={advanced.some((type)=>Boolean(values[type.key]))}><summary>Optional advanced measurements</summary><p className="muted">Add what is useful to you. We put the measurements that improve the widest range of common fit decisions first, then keep the more specialized detail below.</p>{advancedTier("Most useful additional measurements","These are the best next measurements to add after your Core Fit Profile.",mostUseful)}{advancedTier("Additional measurements","Lower-priority detail that can sharpen fit for specific garments and body areas.",additional)}</details>
