-- Fit Report body identity is chronological within each counted identity stream.
-- Compare the submitted body state only to the most recently created report for
-- the same member + Product + normalized size + objective garment variant.
-- Never revive an older historical report merely because the member later
-- returns to a similar body measurement.
...
declare
  ...
  v_meaningful_body_change boolean:=false;
begin
  ...
  -- The latest-created report is the current chronological body episode for
  -- this exact Product/size/objective-variant stream. Revisions do not reorder
  -- that history, so created_at is deliberately used instead of updated_at.
  select
    fr.id,
    fr.closet_item_id,
  ...
  if v_fit_report_id is not null then
    -- Direction does not matter. Any already-established garment-relevant
    -- measurement moving by >=2% from the latest report's rolling baseline is
    -- a new body episode. Missing -> filled remains enrichment, not a split.
    select exists (
      select 1
      ...
      where baseline.fit_report_id=v_fit_report_id
        and abs(current_measurement.value_canonical-baseline.value_canonical)
            / nullif(abs(baseline.value_canonical),0) >= 0.02
    ) into v_meaningful_body_change;

    if v_meaningful_body_change then
      v_fit_report_id:=null;
      v_closet_item_id:=null;
      v_previous_match_profile_version_id:=null;
    end if;
  end if;
