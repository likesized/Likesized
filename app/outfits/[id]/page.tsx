...
export default async function OutfitDetailPage({ params, searchParams }: { params: Params; searchParams: SearchParams }) {
  ...
  const styleNotes=<div className={styles.styleNotes}>
    ...
    <div className={styles.pills}>{(occasionsResult.data??[]).map((row)=><Link prefetch={false} className="filter" key={row.occasion} href={`/explore?view=outfits&scope=all&occasion=${encodeURIComponent(row.occasion)}`}>{OUTFIT_OCCASION_LABELS.get(row.occasion)??row.occasion}</Link>)}{(stylesResult.data??[]).map((row)=><Link prefetch={false} className="filter" key={row.display_tag} href={`/explore?view=outfits&scope=all&style=${encodeURIComponent(row.display_tag)}`}>#{row.display_tag}</Link>)}</div>
