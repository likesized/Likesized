...
export default function OutfitComposer({ closet, initial }: { closet: ClosetOption[]; initial?: InitialOutfit | null }) {
  ...
  async function chooseMain(file?: File) {
    if (!file) return;
    setBusyPhotos(true);
    setPhotoStatus("Optimizing main photo…");
    try {
      ...
      setPhotos((current) => {
        ...
        const copy = [...current];
        copy.splice(mainIndex, 1);
        return [next, ...copy.map((photo) => ({ ...photo, isMain: false }))];
      });
      setPhotoStatus("Main photo ready.");
      mark();
    } catch (error) {
      setPhotoStatus(error instanceof Error ? error.message : "Photo optimization failed.");
    } finally {
      setBusyPhotos(false);
    }
  }
  ...
  async function chooseAdditional(files: FileList | null) {
    ...
    setBusyPhotos(true);
    setPhotoStatus("Optimizing additional photos…");
    try {
      const available = Math.max(0, 6 - photos.length);
      const chosen = [...files].slice(0, available);
      const next: PhotoState[] = [];
      for (const file of chosen) {
        next.push(await makePhoto(file, photos.length === 0 && next.length === 0));
      }
  ...
  function removePhoto(key: string) {
    ...
    if (tagEditorKey === key) setTagEditorKey(null);
    mark();
  }

  function setMain(key: string) {
    setPhotos((current) => {
      const selectedPhoto = current.find((photo) => photo.key === key);
      if (!selectedPhoto) return current;
      return [
        { ...selectedPhoto, isMain: true },
        ...current.filter((photo) => photo.key !== key).map((photo) => ({ ...photo, isMain: false })),
      ];
    });
    mark();
  }
  ...
  function removeHotspot(photoKey: string, closetItemId: string) {
    setPhotos((current) => current.map((photo) =>
      photo.key === photoKey
        ? { ...photo, tags: photo.tags.filter((tag) => tag.closetItemId !== closetItemId) }
        : photo,
    ));
    mark();
  }
    ...
    setPhotos((current) => current.map((photo) =>
      photo.key === photoKey
        ? {
            ...photo,
            tags: [
              ...photo.tags.filter((tag) => tag.closetItemId !== activeTag.closetItemId),
              { closetItemId: activeTag.closetItemId, x, y },
            ],
          }
        : photo,
    ));
  ...
  function copyMainTags(photoKey: string) {
    ...
    setPhotos((current) => current.map((photo) =>
      photo.key === photoKey
        ? { ...photo, tags: source.tags.map((tag) => ({ ...tag })) }
        : photo,
    ));
    mark();
  }
  ...
  function buildFormData() {
    ...
    for (const photo of photos) {
      if (!photo.existingId && photo.displayBlob && photo.feedBlob) {
        formData.append(
          `photo_display__${photo.key}`,
          new File([photo.displayBlob], "display.webp", { type: "image/webp" }),
        );
        formData.append(
          `photo_feed__${photo.key}`,
          new File([photo.feedBlob], "feed.webp", { type: "image/webp" }),
        );
      }
    }
    return formData;
  ...
  async function runSave(kind: "draft" | "publish" | "update", destination?: string) {
    setMessage(null);
    const action = kind === "draft"
      ? saveOutfitDraft
      : kind === "publish" ? publishOutfit : savePublishedOutfit;
  ...
  function save(kind: "draft" | "publish" | "update", destination?: string) {
    startTransition(() => {
      void runSave(kind, destination);
    });
  }
  ...
  function requestPreview() {
    if (!readyToPreview) {
      setMessage("Add a headline, main photo, 1–6 Closet garments, and at least one Occasion before previewing.");
      return;
    }
    setPreview(true);
  ...
  function handleIframeLoad() {
    ...
    try {
      ...
      // Same-origin page may still be navigating.
    }
  }
  ...
  if (preview) {
    return <section className={styles.previewShell}>
      ...
      <div className={styles.detailLayout}>
        <div>
          {mainPhoto ? <div className={styles.previewMain}>
            <img src={mainPhoto.previewUrl} alt="Outfit preview" />
            {mainPhoto.tags.map((tag) => <span
              key={tag.closetItemId}
              className={styles.previewDot}
              style={{ left: `${tag.x * 100}%`, top: `${tag.y * 100}%` }}
            />)}
          </div> : null}
          {photos.length > 1 ? <div className={styles.previewThumbs}>
            {photos.map((photo) => <img key={photo.key} src={photo.previewUrl} alt="Outfit angle preview" />)}
          </div> : null}
        </div>
        <article className={styles.storyCard}>
          <div className={styles.pills}>
            {occasions.map((key) => <span key={key}>
              {OUTFIT_OCCASIONS.find((item) => item.value === key)?.label ?? key}
            </span>)}
          </div>
          {styleTags.length ? <div className={styles.styleLine}>
            {styleTags.map((tag) => <span key={tag}>#{tag}</span>)}
          </div> : null}
          {story ? <p className={styles.storyText}>{story}</p> : null}
          <h3>Explore This Look</h3>
          <div className={styles.tags}>
            {selectedOptions.map((item) => <div className={styles.tag} key={item.id}>
      ...
      <div className={styles.publishBar}>
        <button type="button" className="secondaryButton" onClick={() => setPreview(false)}>← Back to Edit</button>
        <button type="button" className="secondaryButton" disabled={isPending} onClick={() => save("draft")}>Save Draft</button>
        <button type="button" className="primaryButton" disabled={isPending} onClick={() => save("publish")}>
          {isPending ? "Publishing…" : "Publish Outfit →"}
        </button>
      </div>
    </section>;
  }
    ...
    <section className={styles.composer}>
      <div className={styles.sectionCard}>
        <div className={styles.sectionHeading}>
          <div><span className="eyebrow">1 · PHOTOS</span><h2>Show the full look.</h2></div>
          <span className="muted">1 required + up to 5 additional</span>
        </div>
        <div className={styles.uploadGrid}>
          <label className={styles.uploadBox}>
            <strong>Main / front photo</strong>
            <span>This becomes the cover photo.</span>
            <input
              type="file"
              accept="image/jpeg,image/png,image/webp"
              disabled={busyPhotos}
              onChange={(event) => {
                void chooseMain(event.currentTarget.files?.[0]);
          ...
          <label className={styles.uploadBox}>
            ...
            <span>Bulk upload angles, layers, or with/without accessories.</span>
            <input
              type="file"
              multiple
              accept="image/jpeg,image/png,image/webp"
              disabled={busyPhotos || photos.length >= 6}
              onChange={(event) => {
                void chooseAdditional(event.currentTarget.files);
                event.currentTarget.value = "";
              }}
            />
          </label>
        ...
        {photos.length ? <div className={styles.photoEditorList}>
          {photos.map((photo, index) => <article className={styles.photoEditorCard} key={photo.key}>
            <div
              className={styles.photoEditorImage}
              onClick={(event) => imageClick(photo.key, event)}
              role={activeTag?.photoKey === photo.key ? "button" : undefined}
              tabIndex={activeTag?.photoKey === photo.key ? 0 : undefined}
            >
              <img src={photo.previewUrl} alt={photo.isMain ? "Main Outfit photo" : `Outfit photo ${index + 1}`} />
              {photo.tags.map((tag) => <button
                type="button"
                className={styles.hotspotDot}
                style={{ left: `${tag.x * 100}%`, top: `${tag.y * 100}%` }}
                key={tag.closetItemId}
                title={closetById.get(tag.closetItemId)?.label ?? "Tagged garment"}
                onClick={(event) => {
                  event.stopPropagation();
                  removeHotspot(photo.key, tag.closetItemId);
                }}
              />)}
            </div>
            <div className={styles.photoEditorControls}>
              ...
              <div className={styles.miniActions}>
                {!photo.isMain ? <button type="button" onClick={() => setMain(photo.key)}>Set as Main</button> : null}
                {index > 1 ? <button type="button" onClick={() => movePhoto(index, -1)}>↑</button> : null}
                {index > 0 && index < photos.length - 1 ? <button type="button" onClick={() => movePhoto(index, 1)}>↓</button> : null}
                <button type="button" onClick={() => removePhoto(photo.key)}>Remove</button>
              </div>
              <button
                type="button"
                className={styles.tagItemsButton}
                onClick={() => {
                  setTagEditorKey(tagEditorKey === photo.key ? null : photo.key);
                  setActiveTag(null);
                }}
              >
                {tagEditorKey === photo.key ? "Done tagging" : "Tag items in this photo"}
              </button>
              {tagEditorKey === photo.key ? <div className={styles.hotspotEditor}>
                <p>Choose a garment, then tap where it appears in the photo. Tags stay hidden from viewers until they choose <b>View tagged items</b>.</p>
                {!photo.isMain && mainPhoto?.tags.length
                  ? <button type="button" onClick={() => copyMainTags(photo.key)}>Copy tags from main photo</button>
                  : null}
                <div className={styles.hotspotChoices}>
                  {selectedOptions.map((item) => {
                    const placed = photo.tags.some((tag) => tag.closetItemId === item.id);
                    const active = activeTag?.photoKey === photo.key && activeTag.closetItemId === item.id;
                    return <button
                      type="button"
                      className={active ? styles.hotspotActive : placed ? styles.hotspotPlaced : undefined}
                      key={item.id}
                      onClick={() => setActiveTag({ photoKey: photo.key, closetItemId: item.id })}
                    >{placed ? "✓ " : "+ "}{item.label}</button>;
                  })}
                </div>
                {activeTag?.photoKey === photo.key
                  ? <strong className={styles.tapPrompt}>Tap the photo to place this tag.</strong>
                  : null}
              </div> : null}
            </div>
          </article>)}
      ...
      <div className={styles.sectionCard}>
        <div className={styles.sectionHeading}>
          <div><span className="eyebrow">2 · THE POST</span><h2>Give the Outfit a point of view.</h2></div>
        </div>
        <label>
          Headline <span className={styles.counter}>{headline.length}/100</span>
          <input
            value={headline}
            maxLength={100}
            required
            placeholder="A short headline for the Style Feed"
            onChange={(event) => { setHeadline(event.target.value); mark(); }}
          />
        </label>
        <label>
          Outfit Story <span className="muted inlineMuted">optional</span> <span className={styles.counter}>{story.length}/5,000</span>
          <textarea
            value={story}
            rows={9}
            maxLength={5000}
            placeholder="Tell people about the look, layers, accessories, why you styled it this way, or anything else worth knowing."
            onChange={(event) => { setStory(event.target.value); mark(); }}
          />
        </label>
      </div>
      ...
      <div className={styles.sectionCard}>
        <div className={styles.sectionHeading}>
          <div><span className="eyebrow">3 · OCCASION & STYLE</span><h2>Make the look discoverable.</h2></div>
          <span className="muted">Occasion required · choose 1–2</span>
        </div>
        <div className={styles.occasionGrid}>
          {OUTFIT_OCCASIONS.map(({ value, label }) => <button
            type="button"
            aria-pressed={occasions.includes(value)}
            className={occasions.includes(value) ? styles.occasionSelected : styles.occasion}
            key={value}
        ...
        <div className={styles.styleTagEditor}>
          <label>
            Style tags <span className="muted inlineMuted">optional · up to 3</span>
            <div className={styles.styleInputRow}>
              <input
                value={styleDraft}
                maxLength={30}
                placeholder="oldmoney"
              ...
              />
              <button type="button" onClick={addStyleTag} disabled={styleTags.length >= 3}>Add</button>
            </div>
            <span className="fieldHelp">Use your own words. We’ll make them searchable over time. {styleDraft.length}/30</span>
          </label>
          {styleTags.length ? <div className={styles.styleTags}>
            {styleTags.map((tag) => <button type="button" key={tag} onClick={() => removeStyleTag(tag)}>#{tag} ×</button>)}
          </div> : null}
        </div>
      </div>
      ...
      <div className={styles.sectionCard}>
        <div className={styles.sectionHeading}>
          <div><span className="eyebrow">4 · GARMENTS</span><h2>Tag what you’re actually wearing.</h2></div>
          <span className="muted">1–6 required to publish</span>
        </div>
        {closet.length ? <div className={styles.choices}>
          {closet.map((item) => <label className={styles.choice} key={item.id}>
            <input
              type="checkbox"
              checked={selected.has(item.id)}
              disabled={!selected.has(item.id) && selected.size >= 6}
              onChange={(event) => updateSelected(item.id, event.target.checked)}
            />
            <span><strong>{item.label}</strong><small>{item.detail}</small></span>
          </label>)}
        </div> : <div className="emptyState"><strong>Your Closet is empty.</strong><span>Add a Fit Report without leaving this Outfit.</span></div>}
      ...
      <div className={styles.sectionCard}>
        <label className={styles.commentToggle}>
          <input
            ...
            onChange={(event) => { setCommentsEnabled(event.target.checked); mark(); }}
          />
          <span><strong>Allow comments</strong><small>Comments are on by default. You can turn them off or back on later.</small></span>
        </label>
      </div>
      ...
      <div className={styles.composerActions}>
        {isPublished
          ? <button type="button" className="primaryButton" disabled={isPending || busyPhotos} onClick={() => save("update")}>
              {isPending ? "Saving…" : "Save Changes"}
            </button>
          : <>
              <button type="button" className="secondaryButton" disabled={isPending || busyPhotos} onClick={() => save("draft")}>
                {isPending ? "Saving…" : "Save Draft"}
              </button>
              <button type="button" className="primaryButton" disabled={isPending || busyPhotos} onClick={requestPreview}>Preview Outfit →</button>
            </>}
      </div>
    </section>

    {garmentModal ? <div className={styles.modalBackdrop} role="dialog" aria-modal="true" aria-label="Add a missing Closet garment">
      <div className={styles.garmentModal}>
        <div className={styles.modalHeader}>
          <div><span className="eyebrow">ADD A GARMENT</span><strong>Your Outfit stays right where you left it.</strong></div>
          <button type="button" onClick={() => setGarmentModal(false)}>×</button>
        </div>
        <iframe ref={iframeRef} src="/closet/add" title="Add a Fit Report" onLoad={handleIframeLoad} />
      </div>
    </div> : null}

    {leaveHref ? <div className={styles.modalBackdrop} role="dialog" aria-modal="true" aria-label="Unsaved Outfit">
      <div className={styles.leaveModal}>
        <h2>Save your work?</h2>
        <p>You have unsaved changes to this Outfit.</p>
        <div className={styles.leaveActions}>
          <button type="button" className="primaryButton" disabled={isPending} onClick={() => save(isPublished ? "update" : "draft", leaveHref)}>
            {isPublished ? "Save Changes" : "Save Draft"}
          </button>
          <button type="button" className="secondaryButton" onClick={() => { setDirty(false); window.location.assign(leaveHref); }}>Leave Without Saving</button>
          <button type="button" className="textLink" onClick={() => setLeaveHref(null)}>Keep Editing</button>
        </div>
      </div>
    </div> : null}
  </>;
