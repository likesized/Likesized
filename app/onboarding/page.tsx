...
import { FitProfileForm, type BodyMeasurement, type FitCommunity, type MeasurementType } from "@/app/onboarding/FitProfileForm";
...
export default async function OnboardingPage({searchParams}:{searchParams:SearchParams}){
  ...
  const [{data:profile},{data:fitProfile},{data:types},{data:measurements},{data:location}]=await Promise.all([
    supabase.from("profiles").select("username").eq("id",userId).maybeSingle(),
    supabase.from("fit_profiles").select("preferred_unit_system,fit_community,completed_at").eq("user_id",userId).maybeSingle(),
  ...
  return <main className={`onboardingShell ${isInitialSetup?"":heroStyles.revisitShell}`}>
    ...
    <FitProfileForm username={profile?.username??""} initialCity={location?.city??""} initialStateRegion={location?.state_region??""} isInitialSetup={isInitialSetup} unitSystem={unitSystem} fitCommunity={fitCommunity as FitCommunity} types={(types??[]) as MeasurementType[]} measurements={(measurements??[]) as BodyMeasurement[]} errorMessage={errorMessage}/>
  </main>;
}
