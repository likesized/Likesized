...
export default async function ClosetPage({searchParams}:{searchParams:SearchParams}){
  ...
  const [{data,error},{data:unconfirmedData,error:unconfirmedError}]=await Promise.all([
    supabase.from("closet_items").select("id,variant_id,size_label,wears_count,visibility,product:products(id,name,slug,category,garment_type_key,brand:brands(name)),submission:garment_submissions(brand_text,model_text,garment_type_key)").eq("user_id",userId).order("created_at",{ascending:false}),
  ...
  let reports:FitReport[]=[];let photos:FitPhoto[]=[];
  if(ids.length){const [{data:reportData,error:reportError},{data:photoData,error:photoError}]=await Promise.all([
  ...
  ]);if(reportError||photoError)throw new Error("Could not load Closet evidence.");reports=(reportData??[]) as FitReport[];photos=(photoData??[]) as FitPhoto[];}
  const latestReportByItem=new Map<string,FitReport>();for(const report of reports)if(!latestReportByItem.has(report.closet_item_id))latestReportByItem.set(report.closet_item_id,report);
  const photoByItem=new Map<string,string>();for(const row of [...photos].sort((a,b)=>Number(a.photo_role!=="front")-Number(b.photo_role!=="front")))if(!photoByItem.has(row.closet_item_id))photoByItem.set(row.closet_item_id,row.storage_path);
  const signedPhotos=new Map<string,string>();await Promise.all(items.map(async(item)=>{const path=photoByItem.get(item.id);if(!path)return;const {data:signed}=await supabase.storage.from("fit-reference-photos").createSignedUrl(path,60*30);if(signed?.signedUrl)signedPhotos.set(item.id,signed.signedUrl);}));

  const garmentCount=items.length;const strength=Math.min(100,garmentCount*10);const remaining=Math.max(0,10-garmentCount);
  return <main className="pageShell">
    ...
    {added?<div className="authMessage">Garment added to your Closet.</div>:null}{deleted?<div className="authMessage">Garment and its fit history removed.</div>:null}{evidenceAdded?<div className="authMessage">Thanks — your added item information was sent back for review.</div>:null}
    ...
    {items.length?<div className="tableLike">{items.map((item)=>{const product=one<ProductView>(item.product);const brand=one<BrandView>(product?.brand);const submission=one<PendingView>(item.submission);const report=latestReportByItem.get(item.id);const photo=signedPhotos.get(item.id);const followup=followupByItem.get(item.id);const displayBrand=brand?.name||submission?.brand_text||"Brand";const displayName=product?.name||submission?.model_text||"Garment";const displayType=product?.garment_type_key||submission?.garment_type_key;return <div className="closetRow" key={item.id}>
