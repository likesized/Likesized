...
drop function public.save_fit_profile(text, public.unit_system, jsonb);
...
comment on function public.save_fit_profile(text, public.unit_system, jsonb, jsonb) is
  'Atomically replaces the authenticated owner current measurements and private normally-worn size references, then creates/reuses the matching immutable Fit Profile version.';
