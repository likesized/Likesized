...
type FitReport={closet_item_id:string;fit:string;fit_rating:number|null;would_buy_again:boolean|null;created_at:string};
...
function stars(value:number|null|undefined){return value?`${"★".repeat(value)}${"☆".repeat(5-value)}`:"—";}
...
export default async function ClosetPage({searchParams}:{searchParams:SearchParams}){
  ...
  if(ids.length){const [{data:reportData,error:reportError},{data:photoData,error:photoError}]=await Promise.all([
    supabase.from("fit_reports").select("closet_item_id,fit,fit_rating,would_buy_again,created_at").in("closet_item_id",ids).order("created_at",{ascending:false}),
  ...
  return <main className="pageShell">
    ...
    {items.length?<div className="tableLike">{items.map((item)=>{const product=one<ProductView>(item.product);const brand=one<BrandView>(product?.brand);const report=latestReportByItem.get(item.id);const photo=signedPhotos.get(item.id);return <div className="closetRow" key={item.id}>
      ...
      <div><span className="muted">SIZE</span><strong>{item.size_label}</strong></div><div><span className="muted">LATEST FIT</span><strong>{FIT_LABELS[report?.fit||""]||"—"}</strong></div><div><span className="muted">YOUR RATING</span><strong>{stars(report?.fit_rating)}</strong></div><div><span className="muted">VISIBILITY</span><strong>{item.visibility==="shared"?"Shared":"Private"}</strong></div>
    ...
    </div>;})}</div>:<div className="emptyState"><span className="eyebrow">YOUR CLOSET IS EMPTY</span><h2>Start with something you already know fits.</h2><p>Log the product, original size, physical Fit Result and your 1–5 Fit Rating. Choose Shared only when you want other members to browse the fit evidence; uploading a Fit Photo automatically shares that item.</p><Link className="primaryButton" href="/closet/add">Add my first garment →</Link></div>}
  </main>;
}
