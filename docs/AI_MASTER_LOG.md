...
## Exact next action
Finish the external-catalog import adapter, owner quota alerts, and intake integration on this same canonical line; use only free/explicitly enabled provider credentials and do not create paid overage. Then run the full canonical gate and provide a protected preview for owner review. Do not merge or promote production without explicit owner authorization. FAQ owner review and deferred desktop Fit Profile verification remain open.
