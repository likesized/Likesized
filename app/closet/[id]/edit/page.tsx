...
type Report={id:string;fit:string;fit_notes:string|null;would_buy_again:boolean|null;created_at:string};
...
export default async function EditClosetItemPage({params,searchParams}:{params:Params;searchParams:SearchParams}){
  ...
  const [{data:item,error:itemError},{data:reports,error:reportError},{data:photo,error:photoError},{data:mappings},{data:definitions},{data:responses}]=await Promise.all([
    ...
    supabase.from("fit_reports").select("id,fit,fit_notes,would_buy_again,created_at").eq("closet_item_id",id).eq("user_id",userId).order("created_at",{ascending:false}),
  ...
  const errorMessage=error==="photo_requires_shared"?"A fit/reference photo is always shared. Remove the fit photo before making this Closet item private.":error==="invalid_settings"?"Check the Closet settings.":error==="invalid_observation"?"Choose a valid physical Fit Result.":error==="confirm_delete"?"Type DELETE exactly to remove this garment and its fit history.":error==="save_failed"?"That change could not be saved.":null;
  ...
  return <main className="pageShell">
    ...
    <section className="section"><div className="sectionHeading"><div><span className="eyebrow">FIT NOW</span><h2>Log a new fit observation</h2><p>This records how the same garment physically fits your current body. Log failures too—Too Small and Too Big are useful sizing evidence and never reduce the wearer’s body Match %.</p></div></div>
      <form className="garmentForm" action={logFitObservation}><input type="hidden" name="closet_item_id" value={id}/><div className="fieldPair"><label>Overall fit<select name="fit" defaultValue="" required><option value="" disabled>Select physical fit</option><option value="too_small">Too small</option><option value="snug">Snug</option><option value="just_right">Just right</option><option value="relaxed">Relaxed</option><option value="too_big">Too big</option></select></label><label>Would you buy it again?<select name="would_buy_again" defaultValue="unsure"><option value="yes">Yes</option><option value="no">No</option><option value="unsure">Not sure</option></select></label></div><FitDimensionFields garmentType={product?.garment_type_key} dimensions={dimensions} responses={responses??[]}/><label>Fit notes <span className="muted inlineMuted">optional</span><textarea name="fit_notes" maxLength={1000} rows={4} placeholder="How does it fit now?"/></label><button className="primaryButton" type="submit">Save new fit observation</button></form>
    </section>

    <section className="section"><div className="sectionHeading"><div><span className="eyebrow">LOCKED FIT HISTORY</span><h2>{history.length} observation{history.length===1?"":"s"}</h2></div></div>{history.length?<div className="evidenceList">{history.map((report)=><div className="evidence" key={report.id}><div><strong>{dateLabel(report.created_at)}</strong><span>Body snapshot locked at this try-on</span></div><div><span>Fit</span><strong>{FIT_LABELS[report.fit]||report.fit}</strong></div><div><span>Buy again</span><strong>{report.would_buy_again===true?"Yes":report.would_buy_again===false?"No":"—"}</strong></div>{report.fit_notes?<div><span>Notes</span><strong>{report.fit_notes}</strong></div>:null}</div>)}</div>:null}</section>
