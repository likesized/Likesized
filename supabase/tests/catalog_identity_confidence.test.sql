...
grant select on confidence_candidates to authenticated;
...
select is(
  (select p.identity_trust_tier from public.catalog_candidates c join public.products p on p.id=c.resolved_product_id where c.id=(select candidate_id from confidence_candidates where label='consensus')),
  'provisional',
...
select is(
  (select identity_trust_tier from public.products where id=(select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus'))),
  'corroborated',
...
select pg_temp.add_known_wearer('ca000000-0000-4000-8000-000000000003',(select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')),repeat('c',64));
select pg_temp.add_known_wearer('ca000000-0000-4000-8000-000000000004',(select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')),repeat('d',64));
select pg_temp.add_known_wearer('ca000000-0000-4000-8000-000000000005',(select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')),repeat('e',64));
select is(
  (select identity_trust_tier from public.products where id=(select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus'))),
  'established',
...
select public.report_product_item((select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')),'incorrect_information','One later report against five agreeing wearers.');
reset role;
select is((select priority from public.catalog_review_flags where product_id=(select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')) and flag_type='member_report' and status='open'),'low','One isolated report against Established evidence starts Low because an entry error is more likely');

set local role authenticated;
set local request.jwt.claim.role='authenticated';
set local request.jwt.claim.sub='ca000000-0000-4000-8000-000000000002';
select public.report_product_item((select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')),'incorrect_information','Second independent report.');
reset role;
select is((select max(priority) from public.catalog_review_flags where product_id=(select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')) and flag_type='member_report' and status='open'),'medium','A second independent Established conflict escalates to Medium');

set local role authenticated;
set local request.jwt.claim.role='authenticated';
set local request.jwt.claim.sub='ca000000-0000-4000-8000-000000000003';
select public.report_product_item((select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')),'incorrect_information','Third independent report.');
reset role;
select is((select min(priority) from public.catalog_review_flags where product_id=(select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')) and flag_type='member_report' and status='open'),'high','Three independent Established conflicts escalate the Product to High priority');

update public.catalog_review_flags set status='resolved',resolved_at=now() where product_id=(select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')) and status='open';
...
select public.report_product_item((select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')),'other','Single ordinary concern on Verified content.');
reset role;
select is((select priority from public.catalog_review_flags where product_id=(select resolved_product_id from public.catalog_candidates where id=(select candidate_id from confidence_candidates where label='consensus')) and flag_type='member_report' and status='open'),'low','A single ordinary report on Verified content is Low priority');
