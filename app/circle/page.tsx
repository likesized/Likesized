...
import StyleFeedBoard, { type StyleFeedBoardItem } from "./StyleFeedBoard";
...
type OutfitPost={id:string;user_id:string;headline:string|null;story:string|null;caption:string|null;photo_url:string;published_at:string|null;created_at:string;like_count:number;comment_count:number;comments_enabled:boolean};
...
function feedScope(value:string|undefined):FeedScope{return value==="all"?"all":"twins";}
function clean(value:string|undefined,max=80){return(value??"").trim().replace(/\s+/g," ").slice(0,max);}
function normalizeStyleTag(value:string|undefined){return clean(value,30).replace(/^#+/,"").toLowerCase().replace(/[^a-z0-9]/g,"").slice(0,30);}
function matchMap(data:Match[],category:MatchCategory){return new Map(data.filter((row)=>row.match_category===category).map((row)=>[row.user_id,row.match_score]));}
function feedHref(scope:FeedScope,occasion="",styleTag="",qa=false){const params=new URLSearchParams();if(scope==="all")params.set("scope","all");if(occasion)params.set("occasion",occasion);if(styleTag)params.set("style",styleTag);if(qa)params.set("qa","1");const query=params.toString();return query?`/circle?${query}`:"/circle";}
function date(value:string|null,fallback:string){return new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric"}).format(new Date(value??fallback));}
