...
export default function TaggedItemsPanel({items,postId,signedIn}:{items:TaggedItem[];postId:string;signedIn:boolean}){
  ...
  function renderRelevantReport(report:RelevantReport){return <div className={styles.relevantReport} key={report.fitReportId}><div>{report.bodyMatch===null?<><strong>Your Fit Report</strong><span>Your own exact report</span></>:<><strong>{report.bodyMatch}%</strong><span>Body Match{report.isOwn?" · You":""}</span></>}</div><div><span>Size</span><strong>{report.sizeLabel}</strong></div><div><span>Fit Result</span><strong>{report.fitLabel}</strong></div></div>;}
    ...
    {selected?<div className={styles.itemPreviewOverlay} role="dialog" aria-modal="true" aria-label={`${selected.label} quick view`} onClick={()=>setSelectedId(null)}><div className={styles.itemPreviewCard} onClick={(event)=>event.stopPropagation()}>
      ...
      {evidenceOpen&&meta?<>
        ...
        {showRecommendation&&meta.recommendation?<>
          {meta.strongFitReports.length?<div className={styles.relevantReportList}>{meta.strongFitReports.flatMap((group)=>group.fitBreakdown.filter((fit)=>fit.count>0).map((fit)=><div className={styles.relevantReport} key={`${group.sizeLabel}:${fit.fit}`}><div><strong>{fit.count}</strong><span>report{fit.count===1?"":"s"}</span></div><div><span>Size</span><strong>{group.sizeLabel}</strong></div><div><span>Fit Result</span><strong>{fit.fitLabel}</strong></div></div>))}</div>:bestRelevantReport?<div className={styles.relevantReportList}>{renderRelevantReport(bestRelevantReport)}</div>:null}
        </>:bestRelevantReport?<>
          <div className={styles.relevantReportList}>{renderRelevantReport(bestRelevantReport)}</div>
          {additionalRelevantReports.length?<details className={quickStyles.evidenceSection}><summary className={styles.evidenceLink}>View more Relevant Fit Reports →</summary><div className={styles.relevantReportList}>{additionalRelevantReports.map(renderRelevantReport)}</div></details>:null}
        </>:null}
