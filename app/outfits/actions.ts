...
function optimizedPhoto(formData:FormData,name:string,maxBytes:number){
  ...
  return entry instanceof File&&entry.size>0&&entry.type==="image/webp"&&entry.size<=maxBytes?entry:null;
}
...
export async function createOutfit(formData:FormData) {
  ...
  const displayPhoto=optimizedPhoto(formData,"photo_display",DISPLAY_MAX_BYTES);
  const feedPhoto=optimizedPhoto(formData,"photo_feed",FEED_MAX_BYTES);
  if (caption.length>500 || selectedIds.length<1 || selectedIds.length>6) fail("invalid_fields");
  if (!displayPhoto || !feedPhoto) fail("invalid_photo");
  ...
  const photoPath=`${userId}/${postId}/display.webp`;
  const feedPath=`${userId}/${postId}/feed.webp`;
  const {error:displayUploadError}=await supabase.storage.from("outfit-photos").upload(photoPath,await displayPhoto.arrayBuffer(),{contentType:"image/webp",upsert:false});
  if(displayUploadError)fail("save_failed");
  const {error:feedUploadError}=await supabase.storage.from("outfit-photos").upload(feedPath,await feedPhoto.arrayBuffer(),{contentType:"image/webp",upsert:false});
  if(feedUploadError){
  ...
  if(createError){
    await supabase.storage.from("outfit-photos").remove([photoPath,feedPath]);
