...
type ClosetRow = { id: string; size_label: string; product: unknown };
type ProductRecord = { name: string; brand: unknown };
...
export default async function NewOutfitPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [{ data: profile, error: profileError }, { data: closetData, error: closetError }] = await Promise.all([
  ...
  const closetRows = (closetData ?? []) as ClosetRow[];
  ...
  if (closetIds.length) {
    ...
    if (error) throw new Error("Could not load Closet fit evidence.");
  ...
  if (requestedId) {
    ...
    const { data: outfit, error: outfitError } = await supabase
      .from("outfit_posts")
      .select("id,status,headline,story,comments_enabled")
      .eq("id", requestedId)
      .eq("user_id", userId)
      .maybeSingle();
    ...
    if (itemsResult.error || occasionsResult.error || stylesResult.error || photosResult.error) throw new Error("Could not load this Outfit draft.");
    ...
    if (photoIds.length) {
      ...
      if (error) throw new Error("Could not load Outfit photo tags.");
    ...
    await Promise.all(photoRows.map(async (photo) => {
      if (photo.bucket === "outfit-photos") {
        const { data } = supabase.storage.from("outfit-photos").getPublicUrl(photo.display_path);
        urlByPhoto.set(photo.id, data.publicUrl);
      } else {
    ...
    initial = {
      ...
      closetItemIds: (itemsResult.data ?? []).map((item) => item.closet_item_id),
      occasions: (occasionsResult.data ?? []).map((item) => item.occasion),
      styleTags: (stylesResult.data ?? []).map((item) => item.display_tag),
  ...
  return <main className="pageShell">
    <div className="pageTitle rowTitle">
      <div><span className="eyebrow">{initial?.status === "published" ? "EDIT OUTFIT" : "NEW OUTFIT"}</span><h1>{initial?.status === "published" ? "Keep the post current." : "Build the whole look."}</h1><p>Lead with the Outfit, then connect the garments back to real Fit Reports. Your body measurements stay private.</p></div>
      <Link className="secondaryButton" href={initial?.status === "published" && initial.id ? `/outfits/${initial.id}` : "/outfits"}>Back to outfits</Link>
    </div>
    {saved ? <div className="authMessage">Draft saved.</div> : null}
    <OutfitComposer closet={closet} initial={initial} />
  </main>;
}
