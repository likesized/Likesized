...
export default function TaggedItemsPanel({items,postId,signedIn,showCards=true,returnTo: returnToOverride}:{items:TaggedItem[];postId:string;signedIn:boolean;showCards?:boolean;returnTo?:string}){
  ...
  useEffect(()=>{function openFromPhoto(event:Event){const closetItemId=(event as CustomEvent<{closetItemId?:string}>).detail?.closetItemId;if(!closetItemId)return;const item=items.find((entry)=>entry.closetItemId===closetItemId);if(!item)return;if(signedIn)setSelectedId(item.closetItemId);else setGateItem(item);}window.addEventListener("likesized:open-tagged-item",openFromPhoto);return()=>window.removeEventListener("likesized:open-tagged-item",openFromPhoto);},[items,signedIn]);

  function openItem(item:TaggedItem){if(!signedIn){setGateItem(item);return;}setActionError("");setSelectedId(item.closetItemId);}
  ...
  function renderRelevantReport(report:RelevantReport){return <div className={styles.relevantReport} key={report.fitReportId}><div>{report.bodyMatch===null?<strong>Your Fit Report</strong>:<><strong>Best Available Matching Fit Report</strong><MatchPercentageBadge score={report.bodyMatch} label="Body Match"/></>}</div><div><span>Size</span><strong>{report.sizeLabel}</strong></div><div><span>Fit Result</span><strong>{report.fitLabel}</strong></div></div>;}
  function renderStrongReports(groups:StrongFitReport[]){return <><span className={quickStyles.inlineEvidenceLabel}>Strong Fit Reports</span><div className={styles.relevantReportList}>{groups.flatMap((group)=>group.fitBreakdown.filter((fit)=>fit.count>0).map((fit)=><div className={styles.relevantReport} key={`${group.sizeLabel}:${fit.fit}`}><div><strong>{fit.count}</strong><span>report{fit.count===1?"":"s"}</span></div><div><span>Size</span><strong>{group.sizeLabel}</strong></div><div><span>Fit Result</span><strong>{fit.fitLabel}</strong></div></div>))}</div></>;}
  ...
  return <div className={styles.taggedPanel}>
    ...
    {selected?<div className={styles.itemPreviewOverlay} role="dialog" aria-modal="true" aria-label={`${selected.label} quick view`} onClick={()=>setSelectedId(null)}><div className={styles.itemPreviewCard} onClick={(event)=>event.stopPropagation()}>
      ...
      <Link prefetch={false} className={quickStyles.fullEvidenceLink} href={selected.href} data-full-navigation="true">See Full Details →</Link>
