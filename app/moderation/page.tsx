...
export default async function ModerationPage() {
  ...
  const [
    ...
    supabase.from("content_reports").select("id,target_type,target_id,reported_user_id,reason,details,status,created_at,reporter:profiles!content_reports_reporter_id_fkey(username,display_name),reported:profiles!content_reports_reported_user_id_fkey(username,display_name)").eq("status", "open").order("created_at", { ascending: true }),
  ...
  function evidenceScore(candidateId: string) {
    const evidence = submissionsByCandidate.get(candidateId) ?? [];
    return Number(evidence.some((row) => Boolean(row.retailer_url)))
      + Number(evidence.some((row) => Boolean(row.product_photo_storage_path)))
      + Number(evidence.some((row) => Boolean(row.product_label_photo_storage_path)));
  }
  const needsMoreEvidenceRows = candidateRows.filter((candidate) => candidate.status === "needs_more_evidence");
  const needsMoreEvidenceIds = new Set(needsMoreEvidenceRows.map((candidate) => candidate.id));
  const activeCandidateRows = candidateRows
    .filter((candidate) => candidate.status !== "needs_more_evidence")
    .sort((a, b) => {
      const aUnconfirmed = a.identity_confidence === "unconfirmed";
      const bUnconfirmed = b.identity_confidence === "unconfirmed";
      if (aUnconfirmed !== bUnconfirmed) return Number(bUnconfirmed) - Number(aUnconfirmed);
      if (aUnconfirmed && bUnconfirmed) return evidenceScore(b.id) - evidenceScore(a.id);
      return b.submission_count - a.submission_count;
    });
  const activeReviewFlags = reviewFlags.filter((flag) => !flag.candidate_id || !needsMoreEvidenceIds.has(flag.candidate_id));
  ...
  const renderCandidateEvidence = (candidate: Candidate) => {
    const evidence = submissionsByCandidate.get(candidate.id) ?? [];
    return evidence.length ? <div className={styles.dispute}><h3>Recent member evidence</h3>{evidence.map((submission) => <div key={submission.id}>
  ...
  return <main className="pageShell">
    ...
    <section className={styles.summary}>
      <div><strong>{activeCandidateRows.length}</strong><span>Active catalog candidates</span></div>
      <div><strong>{needsMoreEvidenceRows.length}</strong><span>Needs more evidence</span></div>
      <div><strong>{activeReviewFlags.length + productFlags.length}</strong><span>Active catalog flags</span></div>
    ...
    <section>
      ...
      <p className="fieldHelp">Active Unconfirmed items are sorted toward the top when members supplied stronger identity evidence. Retail links, Product Photos, and Product Label / Tag Photos make these cases easier to resolve. Unconfirmed items stay out of the live catalog until review is resolved.</p>
      {activeCandidateRows.length ? <div className={styles.queue}>{activeCandidateRows.map((candidate) => {
        const candidateFlags = flagsByCandidate.get(candidate.id) ?? [];
        const matchingProducts = products.filter((product) => product.garment_type_key === candidate.garment_type_key).map((product) => ({ product, brand: one<Brand>(product.brand) })).filter(({ brand }) => brand?.normalized_name === candidate.normalized_brand).sort((a, b) => Number(b.product.normalized_name === candidate.normalized_model) - Number(a.product.normalized_name === candidate.normalized_model)).slice(0, 20);
        return <article className={styles.report} key={candidate.id}>
          <strong>{candidate.brand_text} · {candidate.model_text}</strong>
          <div className={styles.meta}><span>{label(candidate.garment_type_key)}</span><span>{label(candidate.identity_confidence)}</span><span>{label(candidate.status)}</span>{candidate.identity_confidence === "unconfirmed" ? <span>{evidenceScore(candidate.id)}/3 requested identity evidence types</span> : null}<span>{candidate.submission_count} member submission{candidate.submission_count === 1 ? "" : "s"}</span><span>Last added {candidate.last_submitted_at ? new Date(candidate.last_submitted_at).toLocaleString() : "—"}</span></div>
          {candidateFlags.length ? <div className={styles.dispute}><h3>Open flags</h3>{candidateFlags.map((flag) => <div key={flag.id}><p><b>{label(flag.flag_type)}</b> · {typeof flag.details.reason === "string" ? flag.details.reason : "Review evidence"}</p><form className={styles.actions} action={dismissCatalogFlag}><input type="hidden" name="flag_id" value={flag.id}/><input name="reason" required maxLength={500} placeholder="Why this flag is false or no longer needs review"/><button className={styles.dismiss}>Dismiss flag</button></form></div>)}</div> : null}
          {renderCandidateEvidence(candidate)}
          {matchingProducts.length ? <form className={styles.actions} action={mapCatalogCandidate}><input type="hidden" name="candidate_id" value={candidate.id}/><select name="product_id" required defaultValue=""><option value="" disabled>Map to an existing canonical Product</option>{matchingProducts.map(({ product, brand }) => <option key={product.id} value={product.id}>{brand?.name || "Brand"} · {product.name}</option>)}</select><input name="reason" required maxLength={500} placeholder="Why these submissions belong to this Product"/><button className={styles.dismiss}>Map to Product</button></form> : <p className="fieldHelp">No same-brand/same-type canonical Product candidate was found. Create one only after identity review.</p>}
          <form className={styles.actions} action={createProductFromCandidate}><input type="hidden" name="candidate_id" value={candidate.id}/><input name="canonical_name" required maxLength={180} defaultValue={candidate.model_text} aria-label="Canonical Product name"/><input name="reason" required maxLength={500} placeholder="Evidence supporting creation of one new canonical Product"/><button className={styles.dismiss}>Create Product + map</button></form>
          <form className={styles.actions} action={setCandidateStatus}><input type="hidden" name="candidate_id" value={candidate.id}/><select name="candidate_status" required defaultValue={candidate.status === "needs_review" || candidate.status === "needs_enrichment" ? candidate.status : "pending"}><option value="pending">Pending Product</option><option value="needs_enrichment">Needs Enrichment</option><option value="needs_review">Needs Review</option>{candidate.identity_confidence === "unconfirmed" ? <option value="needs_more_evidence">Needs More Evidence</option> : null}</select><input name="reason" required maxLength={500} placeholder="Status/review note"/><button className={styles.dismiss}>Update queue status</button></form>
        </article>;
      })}</div> : <div className="emptyState"><h2>No active catalog candidates.</h2><p>Unknown member garments will appear here when they need active review.</p></div>}
    </section>

    <section>
      <h2>Needs More Evidence</h2>
      <p className="fieldHelp">These Unconfirmed items were not reasonably identifiable with the evidence available. They are parked outside the active flag queue. The submitting member sees a small private Closet prompt and can send more evidence; doing so automatically returns the item to active review.</p>
      {needsMoreEvidenceRows.length ? <div className={styles.queue}>{needsMoreEvidenceRows.map((candidate) => <article className={styles.report} key={candidate.id}>
        <strong>{candidate.brand_text} · {candidate.model_text}</strong>
        <div className={styles.meta}><span>{label(candidate.garment_type_key)}</span><span>Unconfirmed</span><span>{evidenceScore(candidate.id)}/3 requested identity evidence types</span><span>{candidate.submission_count} member submission{candidate.submission_count === 1 ? "" : "s"}</span></div>
        {renderCandidateEvidence(candidate)}
        <form className={styles.actions} action={setCandidateStatus}><input type="hidden" name="candidate_id" value={candidate.id}/><input type="hidden" name="candidate_status" value="needs_review"/><input name="reason" required maxLength={500} placeholder="Why this item should return to active review"/><button className={styles.dismiss}>Return to Needs Review</button></form>
      </article>)}</div> : <p>No items are waiting for more member evidence.</p>}
    </section>

    <section>
      <h2>Possible duplicates / identity review</h2>
      <p className="fieldHelp">Flags are review evidence only. Needs More Evidence candidates are parked above instead of filling this active queue.</p>
      {activeReviewFlags.length ? <div className={styles.queue}>{activeReviewFlags.map((flag) => {
        ...
        const target = candidate ? `${candidate.brand_text} · ${candidate.model_text}` : product ? `${productBrand?.name || "Brand"} · ${product.name}` : "Catalog evidence";
        return <article className={styles.report} key={flag.id}><strong>{target}</strong><div className={styles.meta}><span>{label(flag.flag_type)}</span><span>{new Date(flag.created_at).toLocaleString()}</span></div><p>{typeof flag.details.reason === "string" ? flag.details.reason : "Review the linked identity evidence before resolving this flag."}</p><form className={styles.actions} action={dismissCatalogFlag}><input type="hidden" name="flag_id" value={flag.id}/><input name="reason" required maxLength={500} placeholder="Why this flag can be dismissed"/><button className={styles.dismiss}>Dismiss flag</button></form></article>;
      })}</div> : <p>No open active catalog review flags.</p>}
    </section>
    ...
    <section>
      ...
      <div className={styles.queue}>
        <article className={styles.report}><strong>Add Brand alias</strong><form className={styles.actions} action={addBrandAlias}><select name="brand_id" required defaultValue=""><option value="" disabled>Choose canonical Brand</option>{aliasBrands.map((brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}</select><input name="alias" required maxLength={120} placeholder="Reviewed alternate Brand spelling"/><input name="reason" required maxLength={500} placeholder="Why this alias is equivalent"/><button className={styles.dismiss}>Add Brand alias</button></form></article>
        <article className={styles.report}><strong>Add Product alias</strong><form className={styles.actions} action={addProductAlias}><select name="product_id" required defaultValue=""><option value="" disabled>Choose canonical Product</option>{products.map((product) => { const brand = one<Brand>(product.brand); return <option key={product.id} value={product.id}>{brand?.name || "Brand"} · {product.name}</option>; })}</select><input name="alias" required maxLength={180} placeholder="Reviewed alternate Item / Model wording"/><input name="reason" required maxLength={500} placeholder="Why this alias is equivalent"/><button className={styles.dismiss}>Add Product alias</button></form></article>
      </div>
    </section>
    ...
    <section>
      ...
      {canonicalPhotos.length ? <div className={styles.queue}>{canonicalPhotos.map((photo) => {
        ...
        return <article className={styles.report} key={photo.id}><img className={styles.preview} src={photo.public_url} alt={`${brand?.name || "Brand"} ${product?.name || "Product"}`}/><strong>{brand?.name || "Brand"} · {product?.name || "Product"}</strong><div className={styles.meta}><span>{label(photo.source_status)}</span><span>{new Date(photo.created_at).toLocaleString()}</span></div><form className={styles.actions} action={removeCanonicalProductPhoto}><input type="hidden" name="photo_id" value={photo.id}/><input name="reason" required maxLength={500} placeholder="Required reason to remove this Product Photo"/><button className={styles.danger}>Remove Product Photo</button></form></article>;
      })}</div> : <p>No canonical Product Photos need review.</p>}
    </section>

    <section>
      <h2>Reported / spam content</h2>
      {rows.length ? <div className={styles.queue}>{rows.map((row) => <article className={styles.report} key={row.id}>{moderatedContent.get(`${row.target_type}:${row.target_id}`)?.imageUrl ? <img className={styles.preview} src={moderatedContent.get(`${row.target_type}:${row.target_id}`)?.imageUrl ?? ""} alt={moderatedContent.get(`${row.target_type}:${row.target_id}`)?.description ?? "Reported member content"}/> : <div className={styles.unavailable}>Reported image is no longer available.</div>}<strong>{label(row.reason)}</strong><div className={styles.meta}><span>{label(row.target_type)}</span><span>Reported by {name(row.reporter)}</span><span>Content owner: {name(row.reported)}</span><span>{new Date(row.created_at).toLocaleString()}</span></div>{row.details ? <p>{row.details}</p> : null}<form className={styles.actions} action={resolveReport}><input type="hidden" name="report_id" value={row.id}/><input name="reason" maxLength={500} required placeholder="Required moderation note"/><button className={styles.dismiss} name="moderation_action" value="dismiss_report">Dismiss report</button><button className={styles.danger} name="moderation_action" value="remove_content">Remove content</button></form></article>)}</div> : <div className="emptyState"><h2>No open reports.</h2><p>The content queue is clear.</p></div>}
    </section>
