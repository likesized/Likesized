...
- Opening a tagged item uses **one canonical middle-layer quick view** whether the action began from Tagged Items or an on-photo hotspot. There is no second impoverished hotspot popup. The quick view shows Product identity/category, useful Relevant Fit Reports, useful Body Match/size/Fit Result context when available, and the FITuition recommendation/confidence context. It has one full-Product navigation action: **View Garment →**. Do not restore competing **See fit evidence** or **Full details** links.
