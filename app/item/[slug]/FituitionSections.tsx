...
import GarmentNotifyButton from "./GarmentNotifyButton";
...
type OwnReport={id:string;user_id:string;product_id:string;normalized_size_id:string|null;size_label:string;fit:RecommendationEvidence["fit"];created_at:string;garment_condition:string;tracked_variation_key:string|null};
...
type StrongGroup={size:string;count:number;fits:Array<{fit:string;label:string;count:number;percent:number}>};

type FituitionData={
  recommendation:ReturnType<typeof recommendSize>;
  mixed:boolean;
  exactCandidates:Candidate[];
  strongGroups:StrongGroup[];
...
function aggregateStrong(rows:Candidate[],sizeFor:(row:Candidate)=>string):StrongGroup[]{
...
const loadFituitionData=cache(async(productId:string,viewerId:string,selectedVariationKey:string|null):Promise<FituitionData>=>{
  ...
  const [{data:productData,error:productError},candidateResult,ownReportResult,sizeResult]=await Promise.all([
    ...
    supabase.from("fit_reports").select("id,user_id,product_id,normalized_size_id,size_label,fit,created_at,garment_condition,tracked_variation_key").eq("user_id",viewerId).order("created_at",{ascending:false}).limit(200),
  ...
  const [candidateMetaResult,profilesResult,productsResult,snapshotResult,attributeResult]=await Promise.all([
    candidateIds.length?supabase.from("fit_reports").select("id,tracked_variation_key,created_at").in("id",candidateIds):Promise.resolve({data:[],error:null}),
  ...
  const exactCandidates=community.filter((row)=>row.evidence_product_id===product.id&&(!selectedVariationKey||metaById.get(row.fit_report_id)?.tracked_variation_key===selectedVariationKey)).sort((a,b)=>b.historical_match_score-a.historical_match_score||b.historical_coverage_percent-a.historical_coverage_percent);
...
function BodyMatchCard({label,row,profileById,sizeFor}:{label:string;row:Candidate;profileById:Map<string,Profile>;sizeFor:(row:Candidate)=>string}){
  const username=profileById.get(row.user_id)?.username?.trim()||null;
  return <div className={styles.evidenceCard}>
    <div><span className="eyebrow">{label}</span>{username?<Link className={styles.personLink} href={`/people/${encodeURIComponent(username)}`} prefetch={false}>@{username}</Link>:<strong>Member</strong>}</div>
    <div><MatchPercentageBadge score={row.historical_match_score} compact/><span>Body Match</span></div>
    <div><strong>Size {sizeFor(row)}</strong><span>Fit Result: {FIT_RESULT_LABELS[row.fit]??row.fit}</span></div>
  </div>;
}

export function FituitionEvidenceFallback(){return <section className={styles.section} aria-busy="true"><div className={styles.sectionHeader}><span className="eyebrow">FITuition</span><h2>Calculating your fit evidence…</h2><p>We’re checking relevant Fit Reports and your Closet history now.</p></div></section>;}

export async function FituitionEvidenceSections({productId,viewerId,slug,selectedVariationKey,retryHref}:{productId:string;viewerId:string;slug:string;selectedVariationKey:string|null;retryHref:string}){
  let data:FituitionData;
  try{data=await loadFituitionData(productId,viewerId,selectedVariationKey);}catch{
    return <section className={styles.section}><div className={styles.sectionHeader}><span className="eyebrow">FITuition</span><h2>Personalized evidence couldn’t finish.</h2><p>The garment page is still usable. Retry FITuition without reloading unrelated page data.</p><Link href={retryHref} prefetch={false}>Retry FITuition →</Link></div></section>;
  }
  const {recommendation,mixed,exactCandidates,strongGroups,profileById,sizeFor}=data;
  const closest=exactCandidates[0]??null;
  const recommendedGroup=recommendation?strongGroups.find((group)=>group.size===recommendation.sizeLabel)??null:null;
  const lowMatches=exactCandidates.slice(0,3);
  const shouldOfferNotify=!recommendation||!closest||closest.historical_match_score<STRONG_FIT_REPORT_MATCH_THRESHOLD||!recommendedGroup;

  return <section className={styles.section}>
    {recommendation?<>
      <div className={styles.sectionHeader}>
        <span className="eyebrow">FITuition</span>
        <h2>FITuition recommends: Size {recommendation.sizeLabel}</h2>
        <p><strong>Confidence: {recommendationConfidenceLabel(recommendation.confidence)}</strong> · Based on {recommendation.sizeEvidenceCount} relevant Fit Report{recommendation.sizeEvidenceCount===1?"":"s"}.{mixed?" Community evidence and your Closet history do not point to exactly the same size, so confidence reflects that split.":""}</p>
      </div>
      {recommendedGroup?<div className={styles.strongBox}><div className={styles.strongHeader}><strong>Aggregate Fit Report evidence</strong><span>{recommendedGroup.count} strong Body Match{recommendedGroup.count===1?"":"es"} wore Size {recommendedGroup.size}</span></div><div className={styles.aggregateList}>{recommendedGroup.fits.map((fit)=><div className={styles.aggregateRow} key={fit.fit}><strong>{fit.count}</strong><span>{fit.label}</span></div>)}</div></div>:null}
      {closest?<BodyMatchCard label="YOUR CLOSEST MATCH" row={closest} profileById={profileById} sizeFor={sizeFor}/>:null}
    </>:<>
      <div className={styles.sectionHeader}>
        <span className="eyebrow">FITuition</span>
        <h2>Not enough evidence yet to confidently recommend a size.</h2>
        <p>{lowMatches.length?"These are the best available Fit Reports for this garment. Lower Body Matches may be less predictive, so FITuition is not forcing a recommendation.":"There are not enough member Fit Reports for this garment yet to make a reliable recommendation."}</p>
      </div>
      {lowMatches.map((row,index)=><BodyMatchCard key={row.fit_report_id} label={index===0?"BEST AVAILABLE MATCH":"AVAILABLE FIT REPORT"} row={row} profileById={profileById} sizeFor={sizeFor}/>) }
    </>}
    {shouldOfferNotify?<GarmentNotifyButton slug={slug} variationKey={selectedVariationKey}/>:null}
