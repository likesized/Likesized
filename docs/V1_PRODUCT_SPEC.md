...
- **FITuition Details is the intermediate in-Outfit evidence layer, not immediate full-page navigation.** The first details click keeps the viewer in the Outfit context. In a low-confidence state it shows the single best/closest exact report first; when more exact reports exist, the full Garment Detail navigation is the path to see the rest. In a recommendation state it shows the aggregated **Strong Fit Reports** exact-variation summary. The intermediate layer contains the explicit **View Garment Details →** / full-evidence navigation that actually leaves the Outfit. The full Garment Detail page is where the broader evidence set, including related/similar tracked variations, belongs.
