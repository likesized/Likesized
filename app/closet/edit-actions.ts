...
function normalizeProductUrl(raw:string){
  ...
  url.searchParams.sort();if(url.pathname.length>1)url.pathname=url.pathname.replace(/\/+$/,"");return url.toString();
}
...
export async function updateClosetSettings(formData:FormData){
  ...
  if(visibility==="private"){
    const {data:photo,error:photoError}=await supabase.from("fit_reference_photos").select("id").eq("closet_item_id",id).eq("user_id",userId).limit(1).maybeSingle();
...
export async function addUnconfirmedIdentityEvidence(formData:FormData){
  ...
  try{
    if(productPhoto){
      ...
      if(error)throw error;newPaths.push(productPath);
    }
    if(labelPhoto){
      ...
      if(error)throw error;newPaths.push(labelPath);
    }
