...
- When the same base Product appears through legitimate distinct tracked variations, the Tagged Items/clicked quick-view path shows the relevant answered variation-defining structured attributes so the member can tell which variation they are looking at. The UI must not label legitimate repeated same-Product tracked variations as duplicates merely because Product, size or color presentation looks similar.
- **Relevant Fit Reports: X means useful personalized exact-item/exact-tracked-variation Fit Report evidence for the current viewer. It never means the total number of Fit Reports attached to the Product.** If seven reports exist but only two are useful personalized exact evidence, the viewer sees `Relevant Fit Reports: 2`; the raw total is not substituted because it is not useful to this decision.
...
- Opening a tagged item uses **one canonical middle-layer quick view** whether the action began from Tagged Items or an on-photo hotspot. There is no second impoverished hotspot popup. The quick view shows Product identity/category, tracked-variation detail when available, useful Relevant Fit Reports, useful Body Match/size/Fit Result context when available, and the FITuition recommendation/confidence context. It has one full-Product navigation action: **View Garment →**. Do not restore competing **See fit evidence** or **Full details** links.
...
- When personalized evidence is genuinely insufficient for a signed-in viewer with a completed Fit Profile, the quick view says so plainly. Lack of a logged-in viewer is never mislabeled as lack of evidence. When no Relevant Fit Reports are available, the quick view may offer a compact **Notify me** text action with the explanation **“FITuition will notify you when people close to your size post a Fit Report for this item.”**
