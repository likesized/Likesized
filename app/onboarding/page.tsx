...
import { FitProfileForm, type BodyMeasurement, type GarmentFitPreference, type GarmentTypePreferenceOption, type MeasurementType, type SizeReference } from "@/app/onboarding/FitProfileForm";
...
export default async function OnboardingPage({searchParams}:{searchParams:SearchParams}){
  ...
  return <main className="onboardingShell">
    ...
    <FitProfileForm username={profile?.username??""} unitSystem={unitSystem} types={(types??[]) as MeasurementType[]} measurements={(measurements??[]) as BodyMeasurement[]} sizeReferences={(sizeReferences??[]) as SizeReference[]} garmentTypes={(garmentTypes??[]) as GarmentTypePreferenceOption[]} fitPreferences={(fitPreferences??[]) as GarmentFitPreference[]} errorMessage={errorMessage}/>
  </main>;
}
