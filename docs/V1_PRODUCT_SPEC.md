...
- On the Outfit tagged-item surface, **Relevant Fit Reports: X means useful personalized other-wearer evidence for the exact Product + exact tracked variation shown.** It never means the total number of Fit Reports attached to the Product. The viewer's own eligible exact-variation Fit Report does **not** increment this visible count and does **not** suppress the zero-report Notify state; that own report remains useful supporting Closet/FITuition evidence internally. Related/similar tracked variations do **not** inflate this Outfit count; those broader reports belong on the full Garment Detail evidence view.
...
- Opening a tagged item uses **one canonical compact quick view** whether the action began from Tagged Items or an on-photo hotspot. There is no second impoverished hotspot popup. The quick view shows Product identity/category, clicked-only tracked-variation detail when available, the exact Relevant Fit Report count, and the appropriate FITuition state.
- **Zero Relevant Fit Reports hard-gates tagged-item recommendation presentation.** The quick view does not show a size recommendation merely because broader Closet History can produce a numerical winner. Instead it says there is not enough useful evidence and offers the compact bell + **Notify me** action with the explanation **“FITuition will notify you when people close to your size post a Fit Report for this item.”** After subscription the compact state is **Notifications on**.
- With **one or more Relevant Fit Reports but insufficient recommendation confidence**, the quick view plainly says FITuition still cannot recommend a size yet, shows **Relevant Fit Reports: X**, and offers one **FITuition Details** action. With enough strong evidence to recommend, the quick view shows the **recommended size + confidence** and that same FITuition Details action.
- The first clicked tagged-garment quick view always keeps a direct **View Garment Detail →** link at the bottom, regardless of FITuition state, including the zero-report Notify state. **FITuition Details remains the intermediate in-Outfit evidence layer, not immediate full-page navigation.** The FITuition Details click keeps the viewer in the Outfit context. In a low-confidence state it shows the single best/closest exact report first; when more exact reports exist, the full Garment Detail navigation is the path to see the rest. In a recommendation state it shows the aggregated **Strong Fit Reports** exact-variation summary. The intermediate layer also contains explicit full Garment Detail/full-evidence navigation. The full Garment Detail page is where the broader evidence set, including related/similar tracked variations, belongs.
- The intermediate evidence layer avoids redundant/debug-like headings. If the viewer's own report is surfaced anywhere as supporting Closet context, identify it simply as **Your Fit Report** with its Size and Fit Result; do not count it as a Relevant Fit Report and do not describe it as “Body Match unavailable.”
