...
-- One wearer has both a target Exact Variant report and a newer weaker category report.
-- Both legitimate situations remain evidence; tier ranking determines their order.
set local role authenticated; set local request.jwt.claim.role='authenticated'; set local request.jwt.claim.sub='b0000000-0000-4000-8000-000000000002';
...
select is((select count(*) from hierarchy_results),7::bigint,'full hierarchy returns every distinct valid Fit Report situation');
select is((select count(distinct user_id) from hierarchy_results),6::bigint,'distinct-situation evidence may contain multiple reports from one wearer');
select is((select count(*) from hierarchy_results where user_id='b0000000-0000-4000-8000-000000000002'::uuid),2::bigint,'wearer with two legitimate observations contributes both evidence rows');
select is((select fit_report_id from hierarchy_results where user_id='b0000000-0000-4000-8000-000000000002'::uuid order by evidence_rank,fit_report_id limit 1),'b7000000-0000-4000-8000-000000000001'::uuid,'Exact Variant observation ranks ahead of the weaker category observation');

select is((select evidence_level::text from hierarchy_results where fit_report_id='b7000000-0000-4000-8000-000000000001'::uuid),'exact_variant','tier 1 label is Exact Variant');
...
select is((select evidence_rank from hierarchy_results where fit_report_id='b7000000-0000-4000-8000-000000000001'::uuid),1,'Exact Variant rank is 1');
...
select is((select string_agg(evidence_level::text,',' order by evidence_rank,fit_report_id) from hierarchy_results),'exact_variant,exact_product,product_family,similar_garments,brand_garment_type,category_fit,category_fit','all distinct situations sort through the locked fallback tier order');
