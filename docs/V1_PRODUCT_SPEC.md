...
- On the Outfit tagged-item surface, **Relevant Fit Reports: X means useful personalized exact-Product + exact-tracked-variation Fit Report evidence for the current viewer.** It never means the total number of Fit Reports attached to the Product. The viewer's own eligible exact-variation Fit Report counts. Related/similar tracked variations do **not** inflate this Outfit count; those broader reports belong on the full Garment Detail evidence view.
...
- The intermediate evidence layer avoids redundant/debug-like headings and does not describe the viewer's own report as “Body Match unavailable.” An own exact report is simply identified as **Your Fit Report** with its Size and Fit Result.
