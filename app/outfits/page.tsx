...
export default async function OutfitsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  if (feed === "all" || followedIds.length) {
    let postQuery = supabase
      .from("outfit_posts")
      .select("id,user_id,headline,photo_url,published_at,created_at,like_count,comment_count")
      .eq("status", "published")
  ...
  if (creatorIds.length) {
    detailJobs.push((async () => {
      const { data, error } = await supabase.from("profiles").select("id,username,display_name").in("id", creatorIds);
      if (error) throw new Error(`Could not load Outfit creators: ${error.message}`);
  ...
  if (postIds.length) {
    detailJobs.push((async () => {
      ...
      if (likeResult.error) throw new Error(`Could not load Outfit likes: ${likeResult.error.message}`);
      if (occasionResult.error) throw new Error(`Could not load Outfit occasions: ${occasionResult.error.message}`);
      if (styleResult.error) throw new Error(`Could not load Outfit style tags: ${styleResult.error.message}`);
  ...
  return <main className="pageShell">
    <div className={`pageTitle rowTitle ${styles.outfitsPageHeader}`}>
      <div><span className="eyebrow">OUTFITS</span><h1>Looks worth saving.</h1><p>Browse outfits, then open a look for the story, tagged items, and fit details.</p></div>
      <div className={styles.creatorHeaderActions}>
        <Link className={styles.quietLink} href="/outfits/drafts">Drafts{drafts.length ? ` (${drafts.length})` : ""}</Link>
        <Link className={styles.compactPrimary} href="/outfits/new">+ New Outfit</Link>
      </div>
    </div>
    ...
    {drafts.length ? <section className={styles.draftStrip}>
      <div className={styles.draftStripHeader}><div><span className="eyebrow">YOUR DRAFTS</span><strong>Pick up where you left off.</strong></div><Link href="/outfits/drafts">View all</Link></div>
      ...
      return <article className={styles.post} key={post.id}>
        ...
        <div className={styles.body}>
          ...
          <div className={styles.pinCreatorRow}>
            ...
            <div>{profile?.username ? <Link className={styles.creatorLink} href={`/people/${profile.username}`}>{name}</Link> : <strong>{name}</strong>}<span>{new Date(post.published_at || post.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span></div>
          </div>
          ...
          <div className={styles.pinSocialRow}>
            <form action={hasLiked ? unlikeOutfit : likeOutfit}><input type="hidden" name="post_id" value={post.id} /><input type="hidden" name="return_to" value={returnTo} /><button className={hasLiked ? styles.likedButton : styles.likeButton} type="submit" aria-pressed={hasLiked}>{hasLiked ? "♥" : "♡"}</button></form>
            <span>{post.like_count}</span><Link href={`/outfits/${post.id}#comments`}>💬 {post.comment_count}</Link>
          </div>
        </div>
      </article>;
    })}</div> : <div className="emptyState"><span className="eyebrow">{feed === "following" ? "NO FOLLOWED OUTFITS YET" : "NO OUTFITS YET"}</span><h2>{feed === "following" ? "The people you follow haven’t posted an Outfit yet." : "No Outfits have been published yet."}</h2>{feed === "following" ? <Link className={styles.quietLink} href="/people">Find people →</Link> : <Link className={styles.compactPrimary} href="/outfits/new">Create an Outfit</Link>}</div>}
  </main>;
}
