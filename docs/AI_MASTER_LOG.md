...
**Status: IN PROGRESS — 0.1/0.2/0.3/0.4/0.6 COMPLETE; 0.5 requires owner cost input.**
...
`supabase/migrations/README.md` and `supabase/schema_contract.md` now define the 14-file migration directory as the only database replay/deployment history. `supabase/schema.sql` and `supabase/storage.sql` are reference/current-state aids only and are not layered onto a fresh migration replay.
...
- Permanent `.github/workflows/ci.yml` added.
- Node 22 is used.
...
- Future main/PR changes run install/typecheck/build through CI.

### 0.5 Fresh database replay — ⏳ OWNER INPUT REQUIRED
Create a disposable Supabase development branch and replay/verify the canonical migration set there. Never destructively replay against the connected main project.

Supabase requires the **organization ID** to quote branch cost, and the tool requires the cost to be explicitly shown/confirmed by the owner before branch creation.

### 0.6 Resolve compile/type failures — ✅ COMPLETE FOR CURRENT HEAD
The first lockfile-producing CI run reached the commit step only after typecheck and Next.js build succeeded, so there are no known current compile/type failures.

**Phase 0 exit criterion:** 0.5 must pass. Everything else is green.
...
## Exact next action
**PHASE 0.5 — obtain the Supabase organization ID, quote/confirm the development-branch cost, create the disposable branch, and verify a clean canonical database replay.**

Do not begin Phase 1 until Phase 0.5 passes.
