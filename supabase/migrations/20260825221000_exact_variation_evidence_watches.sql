...
alter table public.product_evidence_notifications
  add column objective_variant_key text not null default '',
  add column minimum_match_score integer not null default 85 check(minimum_match_score between 1 and 100);

alter table public.product_evidence_notifications drop constraint product_evidence_notifications_pkey;
alter table public.product_evidence_notifications
  add constraint product_evidence_notifications_pkey primary key(user_id,product_id,objective_variant_key);

create or replace function private.calculate_user_snapshot_match_for_product(
  p_user_id uuid,
...
begin
  ...
  with w as (
    ...
    select
      ...
      sum(case when me.value_canonical is not null and hist.value_canonical is not null
        then w.coverage_weight*sqrt(private.fit_measurement_reliability(me.source,me.method)*private.fit_measurement_reliability(hist.source,hist.method))
    ...
    left join public.body_measurements me
      on me.user_id=p_user_id and me.measurement_type_key=w.measurement_type_key
...
begin
  ...
  with qualifying as (
    select watch.user_id,watch.objective_variant_key
    from public.product_evidence_notifications watch
    cross join lateral private.calculate_user_snapshot_match_for_product(
      watch.user_id,
    ...
    where watch.product_id=new.product_id
      ...
      and watch.objective_variant_key=coalesce(new.objective_variant_key,'')
      and hm.match_score>=watch.minimum_match_score
  )
  ...
  where watch.user_id=q.user_id
    ...
    and watch.objective_variant_key=q.objective_variant_key
    and watch.active;
...
drop trigger if exists notify_product_evidence_watchers_after_fit_report on public.fit_reports;
create trigger notify_product_evidence_watchers_after_fit_report
  after insert on public.fit_reports
  for each row execute function private.notify_product_evidence_watchers();

comment on column public.product_evidence_notifications.objective_variant_key is
  'Exact tracked fit-variation identity for a personalized evidence watch.';
comment on column public.product_evidence_notifications.minimum_match_score is
  'Minimum Body Match required before an exact-variation evidence watch fires; quick-view watches use 85.';
comment on table public.product_evidence_notifications is
  'Private per-member exact-variation FITuition watch state. An active watch fires once when a future shared normal-condition Fit Report for the exact Product and tracked variation reaches the watch minimum Body Match, then turns off until the member enables it again.';
