...
export default async function NotificationsPage({searchParams}:{searchParams:SearchParams}){
  ...
  const [{data:profile,error:profileError},{data:fitProfile,error:fitProfileError},{data:notificationData,error:notificationError},{data:unreadData,error:unreadError}]=await Promise.all([
  ...
  const unreadCount=typeof unreadData==="number"?unreadData:Number(unreadData??0);
