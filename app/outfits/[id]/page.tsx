...
type Product = { id: string; name: string; slug: string; image_url: string | null; retailer_url: string | null; brand: unknown };
type Brand = { name: string };
type Comment = { id: string; user_id: string | null; body: string; created_at: string; profile: unknown };
...
export default async function OutfitDetailPage({ params, searchParams }: { params: Params; searchParams: SearchParams }) {
  ...
  if (creatorResult.error || photosResult.error || occasionsResult.error || stylesResult.error || teaserResult.error) throw new Error("Could not load this Outfit.");
  ...
  if (outfit.comments_enabled) {
    if (viewerId) {
      const { data, error } = await supabase.from("outfit_comments").select("id,user_id,body,created_at,profile:profiles(username,display_name)").eq("post_id", id).order("created_at", { ascending: true }).limit(200);
      if (error) throw new Error("Could not load Outfit comments.");
      ...
      comments = ((data ?? []) as PublicComment[]).map((comment) => ({
        ...
        profile: { username: comment.username, display_name: comment.display_name, avatar_url: null },
      }));
    }
  }
  ...
  if (viewerId) {
    ...
    if (closetIds.length) {
      const { data, error } = await supabase.from("fit_reports").select("closet_item_id,size_label,fit,created_at,product:products(id,name,slug,image_url,retailer_url,brand:brands(name))").in("closet_item_id", closetIds).order("created_at", { ascending: false });
    ...
    if (photoRows.length) {
      ...
      if (error) throw new Error("Could not load Outfit hotspots.");
  ...
  if (viewerId && productIds.length) {
    ...
    for (const report of latestByCloset.values()) {
      const product = one<Product>(report.product);
      if (product?.retailer_url && !retailerByProduct.has(product.id)) retailerByProduct.set(product.id, product.retailer_url);
    }
  }
  ...
  return <main className="pageShell">
    ...
    <div className={styles.detailLayout}><OutfitGallery photos={galleryPhotos} garments={garments} canViewTags={Boolean(viewerId)} /><aside className={styles.storyCard}><div className={styles.pills}>{(occasionsResult.data ?? []).map((row) => <span key={row.occasion}>{OUTFIT_OCCASION_LABELS.get(row.occasion) ?? row.occasion}</span>)}</div>{(stylesResult.data ?? []).length ? <div className={styles.styleLine}>{(stylesResult.data ?? []).map((row) => <span key={row.display_tag}>#{row.display_tag}</span>)}</div> : null}{outfit.story ? <p className={styles.storyText}>{outfit.story}</p> : null}<div className={styles.engagementCounts}><span>♥ {outfit.like_count}</span><span>💬 {outfit.comment_count}</span><span>↗ {outfit.share_count}</span></div>
      {!viewerId ? <div className={styles.memberGate}><strong>See how every piece fit.</strong><p>Join or sign in for sizes worn, Fit Results, Fit Report evidence, photo hotspots, LikeLocker actions, and shopping links.</p><Link className="primaryButton" href={`/login?next=${encodeURIComponent(returnTo)}`}>♡ Like · Follow · Comment</Link></div> : null}
