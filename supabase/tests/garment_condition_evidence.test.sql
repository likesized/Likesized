...
select like(pg_get_functiondef('public.get_product_fit_summary(uuid)'::regprocedure),'%garment_condition=''normal''%','product fit summary excludes materially changed garments');
select like(pg_get_functiondef('public.get_product_evidence_candidates(uuid,uuid,integer)'::regprocedure),'%garment_condition=''normal''%','recommendation evidence excludes materially changed garments');
select like(pg_get_functiondef('public.get_product_evidence_candidates(uuid,uuid,integer)'::regprocedure),'%visibility=''shared''%','recommendation evidence keeps Shared Closet boundary');
select like(pg_get_functiondef('public.get_product_evidence_candidates(uuid,uuid,integer)'::regprocedure),'%row_number() OVER (PARTITION BY s.user_id%','evidence remains unique-wearer capped');
