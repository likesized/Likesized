...
import { trackedVariationDetail, trackedVariationDifferences } from "@/lib/tracked-variation";
import ExpandedEvidenceClient from "./ExpandedEvidenceClient";
import styles from "./itemDetail.module.css";

type ProductRecord={id:string;category:string;garment_type_key:string|null;brand_id:string;product_family_id:string|null};
...
type EvidenceProduct={id:string;name:string;slug:string;brand_id:string;product_family_id:string|null;garment_type_key:string|null;category:string};
...
type FituitionData={
  recommendation:ReturnType<typeof recommendSize>;
  mixed:boolean;
  bestExact:Candidate|null;
  strongExact:Candidate[];
  strongGroups:Array<{size:string;count:number;fits:Array<{fit:string;label:string;count:number;percent:number}>}>;
  related:Candidate|null;
  relatedMeta:CandidateMeta|null;
  profileById:Map<string,Profile>;
  sizeFor:(row:Candidate)=>string;
  closetEvidence:RecommendationEvidence[];
  closetExact:OwnReport[];
  closetJustRight:RecommendationEvidence[];
  closetTop:string|null;
...
function dedupeCandidates(rows:Candidate[],metaById:Map<string,CandidateMeta>){
  ...
  for(const row of ordered){const meta=metaById.get(row.fit_report_id);const key=`${row.user_id}:${row.evidence_product_id}:${meta?.tracked_variation_key??row.fit_report_id}`;if(seen.has(key))continue;seen.add(key);result.push(row);}return result;
}
...
const loadFituitionData=cache(async(productId:string,viewerId:string,selectedVariationKey:string|null):Promise<FituitionData>=>{
  const supabase=await createClient();
  const [{data:productData,error:productError},candidateResult,ownReportResult,sizeResult]=await Promise.all([
    supabase.from("products").select("id,category,garment_type_key,brand_id,product_family_id").eq("id",productId).maybeSingle(),
    supabase.rpc("get_product_evidence_candidates",{p_product_id:productId,p_variant_id:null,p_result_limit:300}),
  ...
  if(productError||!productData||candidateResult.error||ownReportResult.error||sizeResult.error)throw new Error("Could not calculate personalized garment evidence.");
  ...
  const [candidateMetaResult,profilesResult,productsResult,snapshotResult,attributeResult]=await Promise.all([
    ...
    allProductIds.length?supabase.from("products").select("id,name,slug,brand_id,product_family_id,garment_type_key,category").in("id",allProductIds):Promise.resolve({data:[],error:null}),
  ...
  return{
    recommendation,
    mixed:Boolean(recommendation?.sourceBreakdown.sourcesAgree===false),
    bestExact,
    strongExact,
    strongGroups:aggregateStrong(strongExact,sizeFor),
    related,
    relatedMeta:related?metaById.get(related.fit_report_id)??null:null,
    profileById,
    sizeFor,
    closetEvidence,
    closetExact:ownReports.filter((report)=>report.garment_condition==="normal"&&report.product_id===product.id&&Boolean(selectedVariationKey&&report.tracked_variation_key===selectedVariationKey)),
    closetJustRight:closetEvidence.filter((row)=>row.fit==="just_right"),
    closetTop:recommendation?.sourceBreakdown.closetTopSizeLabel??null,
    product,
  };
});

export function FituitionRecommendationFallback(){return <><div className="recommendation" role="status" aria-live="polite"><span>OUR FITUITION</span><strong>Calculating your FITuition…</strong><b>Using your current Fit Profile and relevant Fit Reports.</b></div><div className="tiny">The garment is ready while personalized fit evidence finishes calculating.</div></>;}
export function FituitionEvidenceFallback(){return <section className={styles.section} aria-busy="true"><div className={styles.sectionHeader}><span className="eyebrow">SIZE MATCH EVIDENCE</span><h2>Calculating personalized fit evidence…</h2><p>We’re checking the strongest historical Fit Reports and your relevant Closet History now.</p></div></section>;}

export async function FituitionRecommendation({productId,viewerId,selectedVariationKey,retryHref}:{productId:string;viewerId:string;selectedVariationKey:string|null;retryHref:string}){
  try{
    const {recommendation,mixed}=await loadFituitionData(productId,viewerId,selectedVariationKey);
    return recommendation?<><div className="recommendation"><span>OUR FITUITION SUGGESTS</span><strong>{recommendation.sizeLabel}</strong><b>{recommendationConfidenceLabel(recommendation.confidence)}</b></div><div className="tiny">{mixed?<>Mixed evidence: Size Match currently leans <b>{recommendation.sourceBreakdown.communityTopSizeLabel??"another size"}</b>, while your Closet History leans <b>{recommendation.sourceBreakdown.closetTopSizeLabel??"another size"}</b>.</>:recommendation.sourceBreakdown.communityBlend>0&&recommendation.sourceBreakdown.closetBlend>0?<>Size Match evidence and your relevant Closet History point most strongly to this size.</>:recommendation.sourceBreakdown.closetBlend>0?<>Your relevant Closet History provides the strongest current signal.</>:<>Relevant Size Match evidence provides the strongest current signal.</>}</div></>:<><div className="recommendation"><span>OUR FITUITION SUGGESTS</span><strong>—</strong><b>Not enough relevant evidence yet</b></div><div className="tiny">FITuition waits until Size Match and/or Closet evidence can support a size safely.</div></>;
  }catch{
    return <><div className="recommendation" role="alert"><span>OUR FITUITION</span><strong>Couldn’t calculate right now</strong><b>Your garment details are still available.</b></div><div className="tiny"><Link href={retryHref} prefetch={false}>Retry FITuition →</Link></div></>;
  }
}

export async function FituitionEvidenceSections({productId,viewerId,slug,selectedVariationKey,selectedVariationLabel,selectedVariationAnswers,retryHref}:{productId:string;viewerId:string;slug:string;selectedVariationKey:string|null;selectedVariationLabel:string|null;selectedVariationAnswers:Record<string,string>|null;retryHref:string}){
  let data:FituitionData;
  try{data=await loadFituitionData(productId,viewerId,selectedVariationKey);}catch{
    return <section className={styles.section}><div className={styles.sectionHeader}><span className="eyebrow">SIZE MATCH EVIDENCE</span><h2>Personalized evidence couldn’t finish.</h2><p>The garment page is still usable. You can retry the bounded FITuition calculation without reloading unrelated evidence.</p><Link href={retryHref} prefetch={false}>Retry FITuition →</Link></div></section>;
  }
  const {bestExact,strongExact,strongGroups,related,relatedMeta,profileById,sizeFor,closetEvidence,closetExact,closetJustRight,closetTop,product}=data;
  const relatedDifferences=related?trackedVariationDifferences(product.garment_type_key,selectedVariationAnswers,relatedMeta?.garment_answers):[];
  ...
  return <>
    <section className={styles.section}>
      ...
      {bestExact?<><div className={styles.evidenceCard}><div><span className="eyebrow">BEST EXACT VARIATION</span><strong>{selectedVariationLabel??"Exact variation"}</strong><span>{profileById.get(bestExact.user_id)?.display_name||`@${profileById.get(bestExact.user_id)?.username??"member"}`}</span></div><div><MatchPercentageBadge score={bestExact.historical_match_score} compact/><span>Body Match</span></div><div><strong>{sizeFor(bestExact)}</strong><span>Wore · {FIT_RESULT_LABELS[bestExact.fit]??bestExact.fit}</span></div></div>{bestExact.historical_match_score<STRONG_FIT_REPORT_MATCH_THRESHOLD?<p className={styles.explanation}>This is the closest Fit Report we currently have for this exact variation. A lower Body Match does not mean this item will not fit you — it means we do not yet have a report from someone closer to your measurements.</p>:null}</>:<div className={styles.empty}>No Fit Report from another member exists for this exact tracked variation yet. FITuition can still use clearly reduced related evidence and your relevant Closet History.</div>}
      ...
      {related?<div className={`${styles.evidenceCard} ${styles.section}`}><div><span className="eyebrow">CLOSEST RELATED VARIATION</span><strong>{relatedDetail||EVIDENCE_LABELS[related.evidence_level]||"Related tracked variation"}</strong><span>{profileById.get(related.user_id)?.display_name||`@${profileById.get(related.user_id)?.username??"member"}`}</span>{relatedDifferences.length?<div className={styles.relatedDifference}>{relatedDifferences.join(" · ")}</div>:null}</div><div><MatchPercentageBadge score={related.historical_match_score} compact/><span>Body Match</span></div><div><strong>{sizeFor(related)}</strong><span>Wore · {FIT_RESULT_LABELS[related.fit]??related.fit}</span></div></div>:null}
      <ExpandedEvidenceClient slug={slug} variationKey={selectedVariationKey}/>
    </section>
    <section className={styles.section}><div className={styles.sectionHeader}><span className="eyebrow">YOUR CLOSET HISTORY</span><h2>{closetEvidence.length?"What your own history adds":"No relevant Closet history yet"}</h2><p>Your older Fit Reports are compared against your current body state, so personal history is useful evidence without being treated as an automatic perfect match.</p></div>{closetEvidence.length?<div className={styles.closetSummary}>{closetTop?<div className={styles.closetCard}><strong>{closetTop}</strong><span>Your strongest current Closet size signal for this garment context.</span></div>:null}<div className={styles.closetCard}><strong>{closetEvidence.length} relevant report{closetEvidence.length===1?"":"s"}</strong><span>{closetJustRight.length} currently contribute Just Right evidence. {closetExact.length?`${closetExact.length} are from this exact tracked variation.`:""}</span></div></div>:<div className={styles.empty}>As you log garments and Fit Reports, relevant Closet history can strengthen or challenge the community Size Match signal.</div>}</section>
  </>;
}
