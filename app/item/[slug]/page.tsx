...
type Candidate={fit_report_id:string;user_id:string;closet_item_id:string;evidence_product_id:string;evidence_variant_id:string|null;fit_profile_version_id:string;original_size_label:string;normalized_size_id:string|null;fit:RecommendationEvidence["fit"];historical_match_score:number;historical_coverage_percent:number;evidence_level:EvidenceLevel;evidence_rank:number;attribute_overlap:number;directional_fit_support:number|null};
...
export default async function ItemPage({params,searchParams}:{params:Params;searchParams:SearchParams}){
  ...
  return <main className="pageShell">
    ...
    <section className="section"><div className="sectionHeading"><div><span className="eyebrow">BEST EVIDENCE FIRST</span><h2>How this—or the closest relevant garments—fit bodies like yours</h2><p>A high Match with a bad physical Fit Result stays useful evidence. Garments flagged Shrunk, Stretched out, or Altered / Tailored stay in personal history but are excluded from normal-product recommendations.</p></div></div>
      ...
      })}</div>:<div className="emptyState"><span className="eyebrow">NO FIT EVIDENCE YET</span><h2>Be the first useful data point.</h2><p>Good fits and bad fits both help. LikeSized uses exact variant/product evidence first, then family and clearly labeled similar-garment evidence as the user-built database grows.</p></div>}
    </section>
  </main>;
}
