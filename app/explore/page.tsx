...
type Outfit = { id:string; user_id:string; headline:string|null; caption?:string|null; photo_url:string; created_at:string; profile?:unknown; fixture?:boolean };
...
export default async function ExplorePage({searchParams}:{searchParams:SearchParams}){
  ...
  let outfits:Outfit[]=[];
  if(view==="outfits"){
    let query=supabase.from("outfit_posts").select("id,user_id,headline,photo_url,created_at").eq("status","published").order("created_at",{ascending:false}).limit(resultLimit);
    ...
    const result=await query;
    if(result.error){console.error("Explore Outfit query failed",result.error);throw new Error("Could not load outfits.");}
    outfits=(result.data??[]) as Outfit[];
    if(fixtureMode&&!occasion&&!styleTag)outfits=[...(EXPLORE_FIXTURE_OUTFITS as unknown as Outfit[]),...outfits].slice(0,resultLimit);
  ...
  const outfitUserIds=[...new Set(outfits.filter((post)=>!post.fixture).map((post)=>post.user_id))];
  const outfitProfileResult=outfitUserIds.length?await supabase.from("profiles").select("id,username,display_name,avatar_url").in("id",outfitUserIds):{data:[],error:null};
  if(outfitProfileResult.error)console.error("Explore Outfit profile query failed",outfitProfileResult.error);
  const outfitProfileByUser=new Map((((outfitProfileResult.error?[]:outfitProfileResult.data)??[]) as Profile[]).flatMap((person)=>person.id?[[person.id,person] as const]:[]));
  ...
  for(const post of outfits){if(post.fixture)continue;const person=outfitProfileByUser.get(post.user_id);const avatar=currentProfilePhotoUrl(supabase,person?.avatar_url);if(avatar)outfitProfilePhotos.set(post.user_id,avatar);}

  const filterQuery=new URLSearchParams({view,scope});if(category)filterQuery.set("category",category);if(garmentType)filterQuery.set("type",garmentType);if(brandId)filterQuery.set("brand",brandId);if(itemName)filterQuery.set("item",itemName);if(color)filterQuery.set("color",color);if(occasion)filterQuery.set("occasion",occasion);if(styleTag)filterQuery.set("style",styleTag);for(const [key,value] of Object.entries(selectedAttributes))filterQuery.set(`attr_${key}`,value);if(fixtureMode)filterQuery.set("preview","fixtures");if(resultLimit>24)filterQuery.set("limit",String(resultLimit));
  const base=`/explore?${filterQuery.toString()}`;const nextLimit=Math.min(96,resultLimit+24);
  const viewHref=(nextView:string)=>{const next=new URLSearchParams({view:nextView,scope});if(fixtureMode)next.set("preview","fixtures");return `/explore?${next.toString()}`;};
  ...
  return <main className="pageShell">
    ...
    <ExploreSearch fixtures={fixtureMode} initialQuery={outfitFilterLabel??""}/>
    ...
    {view==="outfits"&&outfits.length?<div className={styles.grid}>{outfits.map((post)=>{const person=post.fixture?one<Profile>(post.profile):outfitProfileByUser.get(post.user_id)??null;const name=person?.display_name?.trim()||person?.username||"LikeSized wearer";const score=matchByUser.get(post.user_id);const avatar=post.fixture?currentProfilePhotoUrl(supabase,person?.avatar_url):outfitProfilePhotos.get(post.user_id);const outfitTitle=post.headline??post.caption??null;const outfitHref=`/outfits/${post.id}`;return <article className={styles.card} key={post.id}>{post.fixture?<span className={styles.testBadge}>TEST OUTFIT</span>:null}{post.fixture?(signed.get(post.id)?<img className={styles.outfit} src={signed.get(post.id)} alt={`Outfit by ${name}`}/>:<div className={styles.outfitFallback}>Temporary outfit preview</div>):<Link href={outfitHref} aria-label={`Open ${outfitTitle||"Outfit"}`}>{signed.get(post.id)?<img className={styles.outfit} src={signed.get(post.id)} alt={`Outfit by ${name}`}/>:<div className={styles.outfitFallback}>Outfit photo unavailable</div>}</Link>}<div className={styles.body}><span className="muted">{score?`${score}% Overall Match`:post.fixture?"Test fit context":"LikeSized community"}</span><div className={styles.outfitCreator}>{avatar?<img className={styles.outfitCreatorAvatar} src={avatar} alt=""/>:<span className={styles.outfitCreatorFallback}>{name.slice(0,1).toUpperCase()}</span>}<strong>{name}</strong></div>{outfitTitle?<p>{outfitTitle}</p>:null}{post.fixture?<ProductMiniBrowser href={`/explore/preview?kind=outfit&id=${post.id}`} label={outfitTitle||"Test outfit"}><span className="textLink">Open test outfit →</span></ProductMiniBrowser>:<><Link className="textLink" href={outfitHref}>View Outfit →</Link>{person?.username?<Link className="textLink" href={`/people/${person.username}`}>View wearer →</Link>:null}{post.user_id!==viewerId?<ReportContentForm targetType="outfit_post" targetId={post.id} returnTo={base}/>:null}</>}</div></article>;})}</div>:null}
