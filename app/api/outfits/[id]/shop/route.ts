...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  if(!claims?.claims?.sub){
    const login=new URL("/login",request.url);login.searchParams.set("next",`/outfits/${id}`);return NextResponse.redirect(login);
  }
  ...
  let target=listing.data?.product_url??null;
  if(!target){const product=await supabase.from("products").select("retailer_url").eq("id",productId).maybeSingle();target=product.data?.retailer_url??null;}
  if(!target)return NextResponse.redirect(new URL(`/item/${productId}`,request.url));
  ...
  catch{return NextResponse.redirect(new URL(`/outfits/${id}`,request.url));}
}
