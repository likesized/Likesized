...
export default async function NewOutfitPage({searchParams}:{searchParams:SearchParams}){
  ...
  if(closetResult.error)throw new Error("Could not load your Closet.");
  if(styleSuggestionResult.error)throw new Error("Could not load Style Tag suggestions.");
  ...
  const closet:ClosetOption[]=closetRows.map((item)=>{
    const product=one<ProductRecord>(item.product),brand=one<BrandRecord>(product?.brand),report=reportByItem.get(item.id);
    return{id:item.id,label:`${brand?.name||"Brand"} · ${product?.name||"Garment"}`,detail:`Size ${item.size_label}${report?` · ${FIT_LABELS[report.fit]||report.fit}`:""}`};
  });
