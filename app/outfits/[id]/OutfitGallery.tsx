...
export default function OutfitGallery({photos,garments}:{photos:GalleryPhoto[];garments:GalleryGarment[]}){
  ...
  return <section className={styles.gallery} aria-label="Outfit photo gallery">
    <div className={styles.galleryStage} role="button" tabIndex={0} aria-label={photos.length>1?`Outfit photo ${index+1} of ${photos.length}. Open full-size photo or use Previous and Next to change photos.`:"Open full-size Outfit photo"} onPointerEnter={warm} onFocus={warm} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={()=>{start.current=null;}} onClick={()=>{if(suppressClick.current){suppressClick.current=false;return;}warm();setLightboxOpen(true);}} onKeyDown={(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();setLightboxOpen(true);}else if(event.key==="ArrowRight"){event.preventDefault();move(1);}else if(event.key==="ArrowLeft"){event.preventDefault();move(-1);}}}>
      ...
      {photos.length>1?<><button className={`${styles.galleryNav} ${styles.galleryPrev}`} type="button" aria-label="Previous Outfit photo" onClick={(event)=>{event.stopPropagation();move(-1);}}>‹</button><button className={`${styles.galleryNav} ${styles.galleryNext}`} type="button" aria-label="Next Outfit photo" onClick={(event)=>{event.stopPropagation();move(1);}}>›</button><span className={styles.galleryCounter}>{index+1} / {photos.length}</span></>:null}
