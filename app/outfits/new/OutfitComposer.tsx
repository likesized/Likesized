...
export default function OutfitComposer({ closet, initial, styleSuggestions = [] }: { closet: ClosetOption[]; initial?: InitialOutfit | null; styleSuggestions?: string[] }) {
  ...
  async function makePhoto(file: File, isMain: boolean): Promise<PhotoState> { const { display, feed } = await optimizeFile(file); return { key: newKey(), previewUrl: URL.createObjectURL(display), displayBlob: display, feedBlob: feed, isMain, tags: [] }; }
