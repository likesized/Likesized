...
- **External catalog import is owner-approved and in progress on this same branch.** Manual entry is the final fallback after canonical lookup, barcode/GTIN, product-link extraction, brand + Item name/Style ID shopping search, and approved retailer/feed sources. No provider is enabled by default; no paid overage, automatic plan upgrade, or automatic billing action is authorized. The admin queue must warn at 80%, mark critical at 95%, and record hard-limit stops so the owner decides how to proceed.

## Exact next action
## Active owner goal — Finish New Fit Report intake before any other feature work

Finish the complete New Fit Report intake on this same canonical line before returning to Explore, navigation, moderation, or other deferred work. The owner-approved intake paths are: (1) scan barcode — search LikeSized by UPC first, then automatically import a retail match when absent; (2) text search LikeSized only; (3) an explicit retail-catalog text search when the internal result is not exact; and (4) visible manual entry as the final fallback.

For a retail import, preserve the full provider record privately and materialize every returned manufacturer color/variant for the one canonical Product. Explore color filters must match every available manufacturer color, not only the color in a particular Fit Report. Do not ask the person to choose a color when the import supplies colors; ask only if the provider supplies no color. After a successful import, catalog facts must be filled from the source rather than retyped; the person supplies their exact owned size, Fit Result, condition, optional notes, and optional shared photo. No paid overage, automatic plan upgrade, or automatic billing action is authorized. Then run the full canonical gate and provide a protected preview for owner review. Do not merge or promote production without explicit owner authorization. FAQ owner review and deferred desktop Fit Profile verification remain open.
