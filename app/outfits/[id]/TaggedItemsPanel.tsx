...
type FitMeta={category:string;profileReady:boolean;matchingFitReports:number;objectiveVariantKey:string;recommendation:Recommendation|null;relevantReports:RelevantReport[];strongFitReports:StrongFitReport[];closetEvidenceCount:number};
...
export default function TaggedItemsPanel({items,postId,signedIn}:{items:TaggedItem[];postId:string;signedIn:boolean}){
  ...
  function renderWatchPrompt(item:TaggedItem){return <div className={quickStyles.notifyPrompt}>{watching[item.closetItemId]?<span className={quickStyles.watchState}>Notifications on</span>:<button className={styles.notifyLink} type="button" disabled={Boolean(watchPending[item.closetItemId])} onClick={()=>void requestWatch(item)}>{watchPending[item.closetItemId]?"Saving…":"Notify me"}</button>}<span>FITuition will notify you when people close to your size post a Fit Report for this item.</span></div>;}
  ...
  return <div className={styles.taggedPanel}>
    ...
    {selected?<div className={styles.itemPreviewOverlay} role="dialog" aria-modal="true" aria-label={`${selected.label} quick view`} onClick={()=>setSelectedId(null)}><div className={styles.itemPreviewCard} onClick={(event)=>event.stopPropagation()}>
      ...
      {evidenceOpen&&meta?<>
        <button className={styles.previewBack} type="button" onClick={()=>setEvidenceOpen(false)}>← Back to FITuition</button>
        <div className={styles.evidenceHeading}><span className={styles.fituitionKicker}>RELEVANT FIT REPORTS</span><strong>{selected.label}</strong><span>Relevant Fit Reports · exact tracked variation</span></div>
        {meta.strongFitReports.length?<div className={styles.relevantReportList}>{meta.strongFitReports.map((group)=><div className={styles.relevantReport} key={group.sizeLabel}><div><strong>Size {group.sizeLabel}</strong><span>{group.count} report{group.count===1?"":"s"}</span></div>{group.fitBreakdown.map((fit)=><div key={fit.fit}><span>{fit.fitLabel}</span><strong>{fit.percent}%</strong></div>)}</div>)}</div>:meta.relevantReports.length?<p className="muted">No strong Body Match group yet. Your exact-item history is shown below.</p>:<p className="muted">No Relevant Fit Reports for this exact variation yet.</p>}
        {meta.relevantReports.length?<><div className={styles.evidenceHeading}><strong>Closest exact reports</strong></div><div className={styles.relevantReportList}>{meta.relevantReports.map((report)=><div className={styles.relevantReport} key={report.fitReportId}><div>{report.bodyMatch===null?<><strong>Your Fit Report</strong><span>Exact-item history · Body Match unavailable</span></>:<><strong>{report.bodyMatch}%</strong><span>Body Match{report.isOwn?" · You":""}</span></>}</div><div><span>Size</span><strong>{report.sizeLabel}</strong></div><div><span>Fit Result</span><strong>{report.fitLabel}</strong></div></div>)}</div></>:null}
      </>:<>
        <div className={styles.itemPreviewTop}>{selected.imageUrl?<button className={quickStyles.garmentImageTrigger} type="button" aria-label="Open full-size garment photo" onClick={()=>setImageOpen(true)}><img className={styles.itemPreviewImage} src={selected.imageUrl} alt=""/></button>:<div className={styles.itemPreviewFallback}>{selected.label.slice(0,1).toUpperCase()}</div>}<div className={styles.itemPreviewInfo}><strong>{selected.label}</strong><span>{meta?.category||selected.detail}</span><span>{isLoading?"Relevant Fit Reports: Checking…":fitError?"Relevant Fit Reports: unavailable":meta?.profileReady?`Relevant Fit Reports: ${meta.matchingFitReports}`:"Complete your Fit Profile for personalized evidence."}</span></div></div>
        <section className={styles.fituitionCard} aria-label="FITuition">
          <span className={styles.fituitionKicker}>FITuition</span>
          {isLoading?<><strong className={styles.fituitionHeadline}>Checking your fit evidence…</strong><p>Comparing Size Match evidence with your Fit Profile and Closet history.</p></>:fitError||!meta?<><strong className={styles.fituitionHeadline}>FITuition couldn’t load this evidence.</strong><p>{fitError||"The evidence request did not finish."}</p><button className={styles.notifyLink} type="button" onClick={()=>retryFit(selected)}>Try again</button></>:!meta.profileReady?<><strong className={styles.fituitionHeadline}>Build your Fit Profile to use FITuition.</strong><p>Your measurements stay private.</p></>:meta.recommendation?<><span className={styles.recommendationLabel}>Our FITuition suggests</span><div className={styles.recommendationRow}><strong className={styles.recommendedSize}>{meta.recommendation.sizeLabel}</strong><span className={styles.confidenceBadge}>{meta.recommendation.confidenceLabel}</span></div>{meta.recommendation.sourceBreakdown.sourcesAgree===false?<p><strong>Mixed evidence.</strong> Size Match evidence leans {meta.recommendation.sourceBreakdown.communityTopSizeLabel??"another way"}, while your Closet History leans {meta.recommendation.sourceBreakdown.closetTopSizeLabel??"another way"}.</p>:meta.matchingFitReports>0&&meta.closetEvidenceCount>0?<p>Relevant Fit Reports and your Closet History point most strongly to this size.</p>:meta.matchingFitReports>0?<p>Relevant Fit Reports point most strongly to this size.</p>:meta.closetEvidenceCount>0?<p>Your relevant Closet History provides the strongest current signal. Relevant Fit Reports are still growing.</p>:<p>FITuition used the most relevant Size Match evidence currently available.</p>}{meta.matchingFitReports>0?<button className={styles.evidenceLink} type="button" onClick={()=>setEvidenceOpen(true)}>See Relevant Fit Reports →</button>:renderWatchPrompt(selected)}</>:meta.matchingFitReports>0?<><strong className={styles.fituitionHeadline}>I’m not confident enough to recommend a size yet.</strong><p>I found {meta.matchingFitReports} relevant Fit Report{meta.matchingFitReports===1?"":"s"}, but the combined Size Match and Closet evidence does not point clearly to one size.</p><button className={styles.evidenceLink} type="button" onClick={()=>setEvidenceOpen(true)}>See Relevant Fit Reports →</button></>:<><strong className={styles.fituitionHeadline}>I don’t have enough useful evidence to recommend a size yet.</strong>{renderWatchPrompt(selected)}</>}
        </section>
      ...
      <Link prefetch={false} className={styles.fullDetailsLink} href={selected.href} data-full-navigation="true">View Detailed Garment Report →</Link>
    </div></div>:null}
