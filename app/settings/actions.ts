...
function revalidateProfileSurfaces(){revalidatePath("/settings");revalidatePath("/people","layout");revalidatePath("/search");revalidatePath("/following");revalidatePath("/circle");revalidatePath("/explore");revalidatePath("/closet");revalidatePath("/outfits","layout");revalidatePath("/notifications");}
...
export async function saveUsernameSettings(formData:FormData){
  ...
  const {data:status,error:statusError}=await supabase.rpc("get_username_change_status",{p_username:username});
  if(statusError)fail("username_save_failed");
  const row=Array.isArray(status)?status[0]:status;
  if(!row?.can_change)fail("username_cooldown");
  if(!row?.available)fail("username_taken");
  ...
  if(error){
    ...
    if(error.code==="23514")fail("invalid_username");
    if(error.message?.includes("USERNAME_CHANGE_COOLDOWN"))fail("username_cooldown");
    fail("username_save_failed");
  }
...
export async function saveUnifiedProfileSettings(formData:FormData){
  ...
  const photoEntry=formData.get("profile_photo");const photo=photoEntry instanceof File&&photoEntry.size>0?photoEntry:null;
  if(displayName.length>80||bio.length>300)fail("invalid_profile");
  if(!location||!location.city||!location.state_region)fail("invalid_location");
  ...
  const oldPath=typeof profile?.avatar_url==="string"?profile.avatar_url:null;let newPath:string|null=null;
  if(photo){newPath=`${userId}/${randomUUID()}.webp`;const {error:uploadError}=await supabase.storage.from("profile-photos").upload(newPath,await photo.arrayBuffer(),{contentType:"image/webp",upsert:false});if(uploadError)fail("invalid_profile_photo");}
  const avatarUrl=newPath??(removePhoto?null:oldPath);const now=new Date().toISOString();
  ...
  if(profileResult.error||communityResult.error||locationResult.error){if(newPath&&profileResult.error)await supabase.storage.from("profile-photos").remove([newPath]);fail("profile_save_failed");}
  if(oldPath&&oldPath!==avatarUrl&&oldPath.startsWith(`${userId}/`))await supabase.storage.from("profile-photos").remove([oldPath]);
  revalidateProfileSurfaces();redirect("/settings?profile=saved");
}

export async function saveFollowingNotificationSettings(formData:FormData){
  const enabled=text(formData,"enabled")==="true";const {supabase}=await authenticatedSettingsClient();
