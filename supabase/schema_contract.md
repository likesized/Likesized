...
Canonical CI replays the complete migration directory on a disposable local Supabase database and runs pgTAP tests under `supabase/tests/`. Fit Profile behavior tests cover partial saves, unit normalization, atomic size-reference saves, current-state replacement and immutable-version reuse/change.
