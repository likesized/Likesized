...
- Live migration ledger and GitHub contain 19 migrations through `20260819173357_qualify_current_match_profile_owner.sql`.
...
- During this gate, both the pre-existing `current_vs_historical_fit.test.sql` and the new history test exposed a real matcher defect: `private.calculate_fit_matches_for_profile` used ambiguous unqualified `user_id=v_user_id` inside a RETURNS TABLE PL/pgSQL function.
- Live/canonical fix: `20260819173357_qualify_current_match_profile_owner.sql`, which explicitly qualifies the Fit Profile owner lookup and preserves the existing current-person matching formula/security boundary.
- Final CI must prove the matcher fix plus all current-vs-historical assertions before Phase 1 closes.
...
## Exact next action
**Complete the Phase 1.5 matcher/history-integrity CI gate. If green, mark PHASE 1 complete and proceed immediately to PHASE 2.1 matching verification.**
