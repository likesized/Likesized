...
- Safe already-resolved photo hotspots remain visible logged out.
- Published Outfit identity, comments, tagged-item behavior, Fit Report evidence, and social controls retain the locked behavior already recorded in this specification.
...
## Current production + Roadmap 14 branch reconciliation — 2026-08-28
This is the authoritative current-state override for the older **Roadmap deferred**, **Planned Roadmap 13A**, and **PR #126 / NOT PRODUCTION YET** wording above. Those earlier blocks remain historical context only where they conflict with this section.

### Roadmap 13A — PRODUCTION
Roadmap 13A Automatic Canonical Product Image Scoring is deployed through merged PR #126 at production merge `316063700a657f699d7e697f6ba1e17d04905fbf`. Generic Product representation now uses the shared persisted canonical-image resolver and the locked hierarchy/scoring/admin behavior recorded above. Later Product surfaces must consume that shared resolver instead of creating page-specific image selection.

### Roadmap 14 Garment/Product Detail — OWNER-AUTHORIZED BRANCH CANDIDATE
Draft PR #127 on `product/roadmap-14-garment-product-detail` is the one active Roadmap 14 Product Change. It is branch-only until exact-final Release Verification is green and the owner separately authorizes the identified candidate for production.

Roadmap 14 branch behavior:
- `/item/[slug]` represents the canonical Product and a selected **tracked fit variation**, not a Size/Color/SKU pseudo-variation. Variation labels/details come from the current variation-defining structured-question taxonomy; Size and Color remain outside tracked-variation identity.
- the Product hero consumes the one Roadmap 13A canonical Product-image resolver with the selected tracked variation, preserving the shared exact-variation → broader Product fallback rather than inventing another image-ranking path;
- FITuition continues to use the one canonical recommendation engine and combines community Size Match evidence with the viewer's relevant Closet History. Roadmap 14 does not create a second sizing algorithm;
- visible evidence order is **closest exact tracked variation first → compact Strong Fit Reports aggregate for qualifying exact reports → one closest related tracked variation with the actual controlled variation difference → lazy See more evidence**;
- Body Match remains wearer-body similarity, not garment fit probability. Exact variation, Body Match, Size worn, Fit Result and recommendation confidence stay separate concepts;
- same-person + same-Product + same tracked variation evidence is deduped for recommendation use; related variations are reduced fallback/support and never inflate the exact-variation aggregate;
- expanded evidence is loaded only when requested through a bounded route rather than eagerly dumping every Fit Report on initial page load;
- supplemental profile/product/snapshot/attribute enrichment failures degrade to reduced evidence instead of turning an otherwise valid Product page into a generic server error;
- count-free Product utility actions are **LikeLocker · Wishlist · Shop · Share · Report**. LikeLocker and Wishlist persist in place; zero retailer destinations hide Shop, one valid listing routes directly, and multiple valid listings use a compact retailer picker; reporting stays on the Product page after submission;
- this Roadmap 14 batch does not implement the deferred My Closet lifecycle, Explore/Search redesign, Wish Locker page audit, or the broader Admin audit.

The current Roadmap 14 candidate requires canonical Master reconciliation and full exact-candidate Release Verification before it can be presented for separate production authorization.