...
test("own exact-item history remains recommendation evidence without inflating the visible Relevant count",()=>{
 assert.match(taggedFit,/targetReport\.user_id===viewerId/);assert.match(taggedFit,/const ownHistory:RecommendationEvidence\[\]/);assert.match(taggedFit,/source:"closet" as const/);assert.match(taggedFit,/recommendSize\(\[\.\.\.otherEvidence,\.\.\.ownHistory\]\)/);assert.match(taggedFit,/const otherRelevantExact=relevantExact\.filter\(\(row\)=>row\.user_id!==viewerId\)/);assert.match(taggedFit,/const relevantReports=strongOtherReports/);assert.match(taggedFit,/matchingFitReports:relevantReports\.length/);assert.match(taggedFit,/strongFitReports:strongAggregate\(otherRelevantExact/);assert.doesNotMatch(taggedFit,/const relevantReports=\[\.\.\.ownExactReports/);
});
